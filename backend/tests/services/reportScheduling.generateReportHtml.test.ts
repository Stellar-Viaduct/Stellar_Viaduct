...
const makeDrifts = () => [
  {
    id: "d1", assetCode: "USDC", bridgeName: "Stellar_Viaduct", sourceChain: "ethereum",
    severity: "low", trendDirection: "stable",
    latestRun: { id: "r1", assetCode: "USDC", bridgeName: "Stellar_Viaduct", sourceChain: "ethereum", status: "complete", triageStatus: "none", triageOwner: null, triageNote: null, triagedAt: null, stellarSupply: 1000000, reportedSupply: 1001000, mismatchPercentage: 0.001, discrepancy: 1000, discrepancyAbs: 1000, severity: "low", startedAt: new Date().toISOString(), finishedAt: new Date().toISOString(), attempt: 1, jobId: null, error: null, sourceData: [] },
    previousRunId: null,
  },
];
...
describe("ReportSchedulingService.generateReportHtml", () => {
  ...
  describe("HTML structure", () => {
    it("includes DOCTYPE and report title in generated HTML", async () => {
      ...
      expect(html).toContain("Stellar Viaduct Report");
    });
  ...
  describe("reconciliation section", () => {
    it("renders drift data when available", async () => {
      ...
      expect(html).toContain("Stellar_Viaduct");
      expect(html).toContain("USDC");
    });
