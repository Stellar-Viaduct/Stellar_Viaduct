...
describe("PreferencesService", () => {
  it("validates supported keys and values", () => {
    ...
    expect(() =>
      (service as unknown as { validateSinglePreference: (category: string, key: string, value: unknown) => void }).validateSinglePreference(
        "display",
        "theme",
        "dark"
      )
  ...
  it("migrates legacy schema v1 import payload to v2", async () => {
    const service = new PreferencesService();
    vi.spyOn(
      service as unknown as { recordMigrationHistory: (userId: string, version: number) => Promise<void> },
