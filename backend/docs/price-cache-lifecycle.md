...
The price cache warmup system ensures that the UI has low-latency access to fresh price values by proactively warming the cache on startup and on a scheduled basis. This document describes the complete lifecycle of price caching in the Stellar Viaduct system.
