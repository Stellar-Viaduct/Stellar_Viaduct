...
These examples complement the Swagger UI at `/docs` and focus on the common request patterns used by Stellar Viaduct integrations.
...
```bash
curl -H "x-api-key: $STELLAR_VIADUCT_API_KEY" \
...
const response = await fetch("http://localhost:3000/api/v1/alerts/history?limit=25", {
  headers: {
    "x-api-key": process.env.STELLAR_VIADUCT_API_KEY ?? "",
  },
});
```
...
Stellar Viaduct returns a compact JSON error envelope:
