# Stellar Viaduct Alerting Runbook

## Overview

This runbook provides operational guidance for responding to Stellar Viaduct alerts and outages. It maps alert types to specific response procedures, escalation paths, and recovery steps.
...
1. **Verify the Alert**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/assets/{assetCode}/supply
   
   # Compare with source chain
   curl -X GET https://api.stellarviaduct.io/api/v1/supply-chain/{assetCode}/verification
   ```
...
3. **Check Recent Transactions**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/transactions?assetCode={assetCode}&limit=50
   ```
...
2. **Analyze Supply Chain**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/supply-chain/{assetCode}/breakdown
   ```

3. **Review Audit Logs**
   ```bash
   # Check for suspicious activity
   curl -X GET https://api.stellarviaduct.io/api/v1/admin/audit?resourceType=supply_verification&limit=100
   ```
...
1. **Confirm Downtime**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/bridges/{bridgeId}/health
   
   # Check recent health scores
   curl -X GET https://api.stellarviaduct.io/api/v1/health?bridgeId={bridgeId}&limit=10
   ```
...
2. **Review Monitoring Data**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/circuit-breaker/status
   
   # Review recent alerts
   curl -X GET https://api.stellarviaduct.io/api/v1/alerts?bridgeId={bridgeId}&limit=20
   ```
...
1. **Check Current Health Score**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/health/{bridgeId}
...
2. **Check Related Alerts**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/alerts?bridgeId={bridgeId}&hours=24
   ```
...
3. **If Multiple Bridges Affected**:
   ...
   - Review Stellar Viaduct infrastructure
...
1. **Verify Price Data**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/price-feeds/{assetCode}
   
   # Compare with external sources
   curl -X GET https://api.stellarviaduct.io/api/v1/price-feeds/{assetCode}/comparison
   ```
...
2. **Review Market Activity**
   ```bash
   # Check recent trades
   curl -X GET https://api.stellarviaduct.io/api/v1/pools/{assetCode}/trades?limit=50
   
   # Check liquidity depth
   curl -X GET https://api.stellarviaduct.io/api/v1/pools/{assetCode}/depth
   ```
...
1. **Verify Volume Data**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/analytics/volume?assetCode={assetCode}&period=24h
   
   # Compare with historical average
   curl -X GET https://api.stellarviaduct.io/api/v1/analytics/volume/comparison?assetCode={assetCode}
   ```
...
1. **Analyze Transaction Patterns**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/transactions?assetCode={assetCode}&sort=amount&limit=50
   ```
...
1. **Verify Reserve Status**
   ...
   curl -X GET https://api.stellarviaduct.io/api/v1/bridges/{bridgeId}/reserves
   
   # Get reserve commitments
   curl -X GET https://api.stellarviaduct.io/api/v1/bridge-registry/{bridgeId}/commitments
   ```
...
1. **Review Reserve History**
   ```bash
   # Get reserve history
   curl -X GET https://api.stellarviaduct.io/api/v1/bridges/{bridgeId}/reserves/history?period=7d
   ```
...
- **Contact**: security@stellarviaduct.io
- **Response Time**: Immediate

#### Compliance Team
- **Trigger**: Regulatory issues, suspicious activity
- **Contact**: compliance@stellarviaduct.io
- **Response Time**: < 1 hour

#### Communications Team
- **Trigger**: Public-facing incidents, user impact
- **Contact**: comms@stellarviaduct.io
- **Response Time**: < 30 minutes
...
### Stellar Viaduct Team

| Role | Contact | Escalation Level |
|------|---------|------------------|
| On-Call Engineer | oncall@stellarviaduct.io | Level 1 |
| Engineering Lead | eng-lead@stellarviaduct.io | Level 2 |
| CTO | cto@stellarviaduct.io | Level 3 |
| Security Team | security@stellarviaduct.io | Specialized |
| Compliance | compliance@stellarviaduct.io | Specialized |
| Communications | comms@stellarviaduct.io | Specialized |
...
|---------|------|---------|--------|
| 1.0 | 2024-12-15 | Initial runbook creation | Stellar Viaduct Team |

### Feedback

Submit runbook feedback and improvement suggestions to: runbook-feedback@stellarviaduct.io
...
- **On-Call**: oncall@stellarviaduct.io
- **Security**: security@stellarviaduct.io (PagerDuty)
- **Executive**: exec-oncall@stellarviaduct.io
...
- [Monitoring Dashboard](https://dashboard.stellarviaduct.io)
- [Status Page](https://status.stellarviaduct.io)
