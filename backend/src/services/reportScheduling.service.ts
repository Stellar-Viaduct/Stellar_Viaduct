...
export class ReportSchedulingService {
  ...
  private async buildReconciliationSection(): Promise<string> {
    try {
      const result = await this.reconciliationService.getDriftSummaries({ limit: 10 });
      const drifts = result.summaries;
      const rows = drifts
        .map(
          (d) =>
...
<head>
  <meta charset="UTF-8" />
  <title>Stellar Viaduct Report – ${periodLabel}</title>
  <style>
...
<body>
  <h1>Stellar Viaduct Report</h1>
  <p><strong>Period:</strong> ${periodLabel}</p>
  ${protocolStats}
  ${assetRankings}
  ${alertSummary}
  ${reconciliation}
  <footer>Generated by Stellar Viaduct · ${delivery.frequency} report for ${delivery.email}</footer>
</body>
</html>`;
  }
