...
export class TelegramBotService {
  ...
  private registerCommandHandlers(): void {
    ...
    this.bot.command("start", async (ctx) => {
      try {
        ...
        await ctx.replyWithMarkdown(
          `*Welcome to Stellar Viaduct Bot!* 🌉\n\n` +
            ...
            `Need assistance? Contact support@stellarviaduct.io`
        );
    ...
    this.bot.command("help", (ctx) => {
      ctx.replyWithMarkdown(
        `*Stellar Viaduct Bot Commands* ℹ️\n\n` +
          ...
          `For more info, visit: https://stellarviaduct.io/docs`
      );
    });
  ...
  private async getSystemStatus(): Promise<string> {
    ...
    return (
      ...
      `[Dashboard](https://stellarviaduct.io/dashboard)`
    );
  }
