  ...
  server.get(
    ...
    async (_request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const metrics = [
          `# HELP viaduct_health_status Health check status (1=healthy, 0.5=degraded, 0=unhealthy)`,
          `# TYPE viaduct_health_status gauge`,
          `viaduct_health_status{component="database"} ${health.checks.database.status === "healthy" ? 1 : health.checks.database.status === "degraded" ? 0.5 : 0}`,
          `viaduct_health_status{component="redis"} ${health.checks.redis.status === "healthy" ? 1 : health.checks.redis.status === "degraded" ? 0.5 : 0}`,
          `viaduct_health_status{component="external_apis"} ${health.checks.externalApis.status === "healthy" ? 1 : health.checks.externalApis.status === "degraded" ? 0.5 : 0}`,
          `viaduct_health_status{component="system"} ${health.checks.system.status === "healthy" ? 1 : health.checks.system.status === "degraded" ? 0.5 : 0}`,
          `viaduct_health_status{component="overall"} ${health.status === "healthy" ? 1 : health.status === "degraded" ? 0.5 : 0}`,
          "",
          `# HELP viaduct_uptime_seconds Application uptime in seconds`,
          `# TYPE viaduct_uptime_seconds counter`,
          `viaduct_uptime_seconds ${health.uptime}`,
          "",
          `# HELP viaduct_health_check_duration_seconds Health check duration in seconds`,
          `# TYPE viaduct_health_check_duration_seconds gauge`,
          `viaduct_health_check_duration_seconds{component="database"} ${(health.checks.database.duration || 0) / 1000}`,
          `viaduct_health_check_duration_seconds{component="redis"} ${(health.checks.redis.duration || 0) / 1000}`,
          `viaduct_health_check_duration_seconds{component="external_apis"} ${(health.checks.externalApis.duration || 0) / 1000}`,
          `viaduct_health_check_duration_seconds{component="system"} ${(health.checks.system.duration || 0) / 1000}`,
          "",
        ];
