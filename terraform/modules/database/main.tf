...
# Purpose: Provision a managed RDS instance configured for Stellar Viaduct analytics.
...
resource "aws_security_group" "rds" {
  name        = "stellar-viaduct-${var.environment}-rds-sg"
  vpc_id      = var.vpc_id
  description = "Security group for Stellar Viaduct database"
...
resource "aws_db_subnet_group" "main" {
  name       = "stellar-viaduct-${var.environment}-db-subnet-group"
...
resource "aws_db_parameter_group" "postgresql" {
  name        = "stellar-viaduct-parameter-group"
  family      = "postgres15"
  description = "Stellar-Viaduct RDS parameter group with TimescaleDB support"
...
resource "aws_db_instance" "main" {
  ...
  tags = {
    Name = "stellar-viaduct-${var.environment}-db"
  }
}
