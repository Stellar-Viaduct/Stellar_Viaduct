...
resource "aws_security_group" "alb" {
  name        = "stellar-viaduct-${var.environment}-alb-sg"
  vpc_id      = var.vpc_id
  description = "Security group for Stellar Viaduct Load Balancer"
...
resource "aws_lb" "main" {
  name               = "stellar-viaduct-${var.environment}-alb"
  internal           = false
...
resource "aws_lb_target_group" "ingress" {
  name     = "stellar-viaduct-${var.environment}-tg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = var.vpc_id
