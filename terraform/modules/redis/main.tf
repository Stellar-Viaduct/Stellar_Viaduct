...
resource "aws_security_group" "redis" {
  name        = "stellar-viaduct-${var.environment}-redis-sg"
  vpc_id      = var.vpc_id
  description = "Security group for Stellar Viaduct Redis cluster"
...
resource "aws_elasticache_subnet_group" "main" {
  name       = "stellar-viaduct-${var.environment}-redis-subnet-group"
...
resource "aws_elasticache_replication_group" "main" {
  replication_group_id          = "stellar-viaduct-${var.environment}-redis"
  replication_group_description = "Stellar-Viaduct Redis cluster for caching and BullMQ jobs"
  ...
  tags = {
    Name = "stellar-viaduct-${var.environment}-redis"
  }
}
