...
variable "alert_email" {
  ...
  type        = string
  default     = "ops@stellarviaduct.io"
}

# CloudWatch Log Group for Application
resource "aws_cloudwatch_log_group" "app" {
  name              = "/stellar-viaduct/${var.environment}/backend"
  retention_in_days = 30

  tags = {
    Name = "stellar-viaduct-${var.environment}-logs"
  }
}

# SNS Topic for Alerts
resource "aws_sns_topic" "alerts" {
  name = "stellar-viaduct-${var.environment}-alerts"
...
resource "aws_cloudwatch_dashboard" "system" {
  dashboard_name = "stellar-viaduct-${var.environment}"
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            ["AWS/RDS", "CPUUtilization", "DBInstanceIdentifier", "stellar-viaduct-${var.environment}-db"]
          ]
          period = 300
          stat   = "Average"
          region = "us-east-1"
