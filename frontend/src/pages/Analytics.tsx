...
export default function Analytics() {
  ...
  const [selectedSymbols, setSelectedSymbols] = useLocalStorageState<string[]>(
    "stellar-viaduct:analytics-compare:v1",
    []
  );
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="px-4 py-2 bg-viaduct-accent text-white rounded-lg text-sm font-semibold hover:bg-viaduct-accent/90 transition-colors disabled:opacity-50"
        >
          Capture Snapshot
        </button>
      </div>
      ...
      {snapshot && (
        <section className="print:m-0 print:p-0">
          <div className="flex justify-between items-center mb-4 no-print">
            ...
            <button onClick={() => setSnapshot(null)} className="text-sm text-viaduct-text-secondary hover:text-white">
              Dismiss
            </button>
          </div>
          <SnapshotCard 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.slice(0, 4).map((stat) => (
          <div
            key={stat.id}
            className="bg-viaduct-card border border-viaduct-border rounded-lg p-6 cursor-pointer hover:border-viaduct-accent transition-colors"
        ...
        <button
          type="button"
          onClick={() => setIsDrilldownOpen(true)}
          className="bg-viaduct-card border border-viaduct-border rounded-lg p-6 flex flex-col items-center justify-center hover:border-viaduct-accent transition-colors"
        >
          <span className="text-3xl mb-2">🔍</span>
          <span className="text-sm font-medium text-viaduct-text-primary">View All Metrics</span>
        </button>
      </div>
      ...
      <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
        ...
        <div className="mt-4">
          {error ? (
            ...
            <p className="text-viaduct-text-secondary" role="status" aria-live="polite">
              Loading assets…
            </p>
              ...
              {assetsData.map((asset) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-md border px-3 py-2 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent ${
                      selected
                        ? "border-viaduct-accent bg-viaduct-accent/20 text-white"
                        : "border-viaduct-border bg-viaduct-background text-viaduct-text-secondary hover:text-white"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {selectedAssets.map((asset, index) => {
                ...
                return (
                  <article
                    key={asset.symbol}
                    className="bg-viaduct-background border border-viaduct-border rounded-lg p-4"
                    aria-label={`${asset.symbol} comparison metrics`}
                  >
                    <h3 className="text-lg font-semibold text-white">{asset.symbol}</h3>
                    <p className="text-sm text-viaduct-text-secondary">{asset.name}</p>

                    <dl className="mt-4 space-y-2 text-sm">
                      <div className="flex justify-between gap-3">
                        <dt className="text-viaduct-text-secondary">Health Score</dt>
                        <dd className="text-white font-medium">
                          {asset.health?.overallScore ?? "--"}
                        </dd>
                      </div>
                      <div className="flex justify-between gap-3">
                        <dt className="text-viaduct-text-secondary">Trend</dt>
                        <dd className="text-white font-medium">
                          {asset.health?.trend ?? "--"}
                        </dd>
                      </div>
                      <div className="flex justify-between gap-3">
                        <dt className="text-viaduct-text-secondary">VWAP</dt>
                        <dd className="text-white font-medium">
                          {typeof vwap === "number" ? `$${vwap.toFixed(4)}` : "--"}
                        </dd>
                      </div>
                      <div className="flex justify-between gap-3">
                        <dt className="text-viaduct-text-secondary">Price Sources</dt>
                        <dd className="text-white font-medium">
