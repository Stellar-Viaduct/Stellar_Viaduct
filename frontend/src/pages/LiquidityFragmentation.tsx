...
export default function LiquidityFragmentation() {
  const [selectedAsset, setSelectedAsset] = useLocalStorageState<string>(
    "stellar-viaduct:fragmentation-asset:v1",
    "USDC"
  );
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <select
          ...
          className="bg-stellar-bg border border-viaduct-border rounded-lg px-4 py-2 text-white"
        >
      ...
      {metrics && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-4">
            <p className="text-xs text-viaduct-text-secondary">Fragmentation Score</p>
            <p className={`mt-1 text-xl font-bold ${getFragmentationColor(metrics.fragmentationScore)}`}>
              {metrics.fragmentationScore.toFixed(2)}
            </p>
            <p className="text-xs text-viaduct-text-secondary mt-1">
              {getFragmentationLabel(metrics.fragmentationScore)}
            </p>
          </div>

          <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-4">
            <p className="text-xs text-viaduct-text-secondary">Total Liquidity</p>
          ...
          <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-4">
            <p className="text-xs text-viaduct-text-secondary">Herfindahl Index</p>
            <p className="mt-1 text-xl font-bold text-white">{metrics.herfindahlIndex.toFixed(4)}</p>
            <p className="text-xs text-viaduct-text-secondary mt-1">Concentration measure</p>
          </div>

          <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-4">
            <p className="text-xs text-viaduct-text-secondary">DEX Count</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
              ...
              {distribution.map((dex) => (
                <div key={dex.dex} className="flex items-center justify-between">
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <span className="text-sm text-viaduct-text-secondary">{dex.dex}</span>
        ...
        <section className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-viaduct-text-secondary mb-1 block">From</label>
                <select
                  value={routeParams.fromAsset}
                  onChange={(e) => setRouteParams({ ...routeParams, fromAsset: e.target.value })}
                  className="w-full bg-stellar-bg border border-viaduct-border rounded px-3 py-2 text-white text-sm"
                >
                  {SUPPORTED_ASSETS.map((asset) => (
                    <option key={asset} value={asset}>
                      {asset}
              ...
              <div>
                <label className="text-xs text-viaduct-text-secondary mb-1 block">To</label>
                <select
            ...
            <div>
              <label className="text-xs text-viaduct-text-secondary mb-1 block">Amount</label>
              <input
                type="number"
              ...
              <div className="mt-4 space-y-3">
                <div className="bg-stellar-bg rounded p-3">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs text-viaduct-text-secondary">Estimated Output</span>
                    <span className="text-sm font-medium text-white">
                      {optimalRoute.estimatedOutput.toFixed(4)} {optimalRoute.toAsset}
                    </span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs text-viaduct-text-secondary">Slippage</span>
                    <span className="text-sm font-medium text-yellow-400">
                      {optimalRoute.estimatedSlippage.toFixed(2)}%
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-viaduct-text-secondary">Price Impact</span>
                ...
                <div className="space-y-2">
                  <p className="text-xs text-viaduct-text-secondary font-medium">Route Steps:</p>
                  {optimalRoute.routes.map((step, idx) => (
                    <div key={idx} className="bg-stellar-bg rounded p-2 text-xs">
                      <div className="flex justify-between">
                        <span className="text-viaduct-text-secondary">{step.dex}</span>
                        <span className="text-white font-medium">
                          {(step.share * 100).toFixed(1)}%
                        </span>
                      </div>
                      <div className="text-viaduct-text-secondary mt-1">
      ...
      <section className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
        ...
        {isLoading ? (
          ...
          <p className="text-viaduct-text-secondary text-sm py-8 text-center">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-viaduct-border">
                  <th className="text-left py-2 px-3 text-viaduct-text-secondary font-medium">
                    Pair
                  </th>
                  <th className="text-left py-2 px-3 text-viaduct-text-secondary font-medium">
                    Buy DEX
                  ...
                  <th className="text-right py-2 px-3 text-viaduct-text-secondary font-medium">
                    Spread
                  </th>
                  <th className="text-right py-2 px-3 text-viaduct-text-secondary font-medium">
                    Profit Est.
              ...
              <tbody>
                {arbitrageOps.map((op, idx) => (
                  <tr key={idx} className="border-b border-viaduct-border/50">
                    <td className="py-2 px-3 text-white font-medium">{op.assetPair}</td>
                    <td className="py-2 px-3 text-viaduct-text-secondary">{op.buyDex}</td>
                    <td className="py-2 px-3 text-viaduct-text-secondary">{op.sellDex}</td>
