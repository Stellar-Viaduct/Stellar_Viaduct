...
function BridgeVerificationRow({
  ...
  return (
    <div className="rounded-xl border border-viaduct-border bg-viaduct-surface">
      <button
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-viaduct-text-primary truncate">{result.bridgeName}</p>
            <p className="text-xs text-viaduct-text-secondary mt-0.5">{result.bridgeId}</p>
          </div>
        </div>

        <div className="hidden sm:flex items-center gap-6 flex-shrink-0 text-sm">
          <div className="text-right">
            <p className="text-viaduct-text-secondary text-xs">Source chain</p>
            <p className="text-viaduct-text-primary capitalize">{result.sourceChain}</p>
          </div>
          <div className="text-right">
            <p className="text-viaduct-text-secondary text-xs">Mismatch</p>
            <p
              className={
                result.mismatchPct > result.mismatchThreshold
                  ? "text-red-400 font-medium"
                  : "text-viaduct-text-primary"
              }
            >
          ...
          <div className="text-right">
            <p className="text-viaduct-text-secondary text-xs">Merkle proof</p>
            <MerkleProofBadge valid={result.merkleProofValid} />
          </div>
          <div className="text-right">
            <p className="text-viaduct-text-secondary text-xs">Cache</p>
            <p className="text-viaduct-text-primary">{result.cacheHit ? "Hit" : "Fresh"}</p>
          </div>
        </div>

        <svg
          className={`w-4 h-4 text-viaduct-text-secondary flex-shrink-0 mt-1 transition-transform ${expanded ? "rotate-180" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      {expanded && (
        <div className="border-t border-viaduct-border px-4 pb-4 pt-3 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-lg bg-viaduct-background p-3 space-y-2">
              <p className="text-xs font-semibold text-viaduct-text-secondary uppercase tracking-wider">
                Stellar State
              </p>
              <div className="flex justify-between text-sm">
                <span className="text-viaduct-text-secondary">Asset</span>
                <span className="text-viaduct-text-primary font-mono">{result.stellar.assetCode}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-viaduct-text-secondary">Supply</span>
                <span className="text-viaduct-text-primary font-mono">
                  {compact(result.stellar.supply)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-viaduct-text-secondary">Issuer</span>
                <span className="text-viaduct-text-primary font-mono text-xs truncate max-w-[150px]">
            ...
            <div className="rounded-lg bg-viaduct-background p-3 space-y-2">
              ...
              {result.ethereum ? (
                <>
                  <div className="flex justify-between text-sm">
                    <span className="text-viaduct-text-secondary">Locked</span>
                    <span className="text-viaduct-text-primary font-mono">
                      {compact(result.ethereum.formattedAmount)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-viaduct-text-secondary">Block</span>
                    <span className="text-viaduct-text-primary font-mono">
                      {result.ethereum.blockNumber.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-viaduct-text-secondary">Paused</span>
                    <span
                      className={
                ...
                <p className="text-viaduct-text-secondary text-sm">No EVM data available</p>
              )}
            </div>
          </div>
          ...
          <div className="rounded-lg bg-viaduct-background p-3 space-y-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              <div>
                <p className="text-viaduct-text-secondary text-xs">Sequence</p>
                <p className="text-viaduct-text-primary font-mono">
              ...
              <div>
                <p className="text-viaduct-text-secondary text-xs">State Consistent</p>
                <p className={result.stateConsistent ? "text-green-400" : "text-red-400"}>
                  {result.stateConsistent ? "Yes" : "No"}
                </p>
              </div>
              <div>
                <p className="text-viaduct-text-secondary text-xs">Threshold</p>
                <p className="text-viaduct-text-primary">{pct(result.mismatchThreshold)}</p>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          onClick={() => refetch()}
          className="flex-shrink-0 rounded-lg border border-viaduct-border bg-viaduct-surface px-4 py-2 text-sm font-medium text-viaduct-text-primary hover:bg-stellar-border/40 transition-colors"
        >
          Refresh all
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total bridges", value: summary.total, color: "text-viaduct-text-primary" },
          ...
          <div
            key={label}
            className="rounded-xl border border-viaduct-border bg-viaduct-surface p-4"
          >
            <p className="text-sm text-viaduct-text-secondary">{label}</p>
      ...
      {isLoading && (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-20 rounded-xl border border-viaduct-border bg-viaduct-surface animate-pulse"
            />
          ))}
        </div>
      )}
      ...
      {!isLoading && results.length === 0 && !isError && (
        <div className="rounded-xl border border-viaduct-border bg-viaduct-surface p-8 text-center text-viaduct-text-secondary">
