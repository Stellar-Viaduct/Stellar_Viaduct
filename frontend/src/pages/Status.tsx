    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-2 text-viaduct-text-secondary">
          API availability and bridge connectivity signals used by Stellar Viaduct.
        </p>
      </header>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        >
          ...
          {healthQuery.isLoading && (
            <p className="text-viaduct-text-secondary text-sm">Checking health endpoint…</p>
          )}
          {healthQuery.isError && (
          ...
          {healthQuery.data && (
            <ul className="space-y-2 text-sm">
              ...
              <li className="text-viaduct-text-secondary">
        ...
        <section
          className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 md:col-span-2"
        ...
        >
          ...
          {bridgesQuery.isLoading && (
            <p className="text-viaduct-text-secondary text-sm">Loading bridge statuses…</p>
          )}
          ...
          {bridgesQuery.data && bridgesQuery.data.length === 0 && (
            <p className="text-viaduct-text-secondary text-sm">No bridges configured.</p>
          )}
          {bridgesQuery.data && bridgesQuery.data.length > 0 && (
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {bridgesQuery.data.map((b) => (
                    <tr key={b.name} className="border-b border-viaduct-border/60">
                      <td className="py-3 pr-4 text-white">{b.name}</td>
                      <td className="py-3 pr-4">
                        <span className="inline-flex items-center gap-2 text-viaduct-text-secondary">
                          {statusDot(b.status)}
                          <span className="capitalize">{b.status}</span>
                        </span>
                      </td>
                      <td className="py-3 text-viaduct-text-secondary tabular-nums">
