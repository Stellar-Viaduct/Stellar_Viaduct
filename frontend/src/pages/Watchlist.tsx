    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-viaduct-text-secondary mt-1">
      ...
      {activeWatchlist && (
        <div className="bg-viaduct-card border border-viaduct-border rounded-lg overflow-hidden shadow-lg">
          <div className="p-6 border-b border-viaduct-border">
            <h2 className="text-xl font-bold text-white flex items-center gap-3">
              {activeWatchlist.name}
              <span className="text-sm font-normal text-viaduct-accent bg-viaduct-background px-2 py-0.5 rounded-full">
                Active
              </span>
            </h2>
          </div>
          ...
          {activeWatchlist.assets.length === 0 ? (
            <div className="p-12 text-center text-gray-400">
              <div className="inline-block p-4 rounded-full bg-viaduct-background mb-4">
              ...
              <Link to="/dashboard" className="mt-4 inline-block text-viaduct-accent hover:text-blue-300">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-viaduct-background border-b border-viaduct-border">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-viaduct-text-secondary uppercase tracking-wider">
                      Asset
                    </th>
                    <th className="px-6 py-4 text-right text-xs font-semibold text-viaduct-text-secondary uppercase tracking-wider">
                      Price (USD)
                    </th>
                    <th className="px-6 py-4 text-right text-xs font-semibold text-viaduct-text-secondary uppercase tracking-wider">
                      Health Score
                ...
                <tbody className="divide-y divide-viaduct-border">
                  {activeWatchlist.assets.map((symbol) => {
                    ...
                    return (
                      <tr key={symbol} className="hover:bg-viaduct-surface/50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <Link to={`/assets/${symbol}`} className="flex items-center gap-3 group">
                            <div className="w-8 h-8 rounded-full bg-viaduct-accent/10 flex items-center justify-center border border-viaduct-accent/20 group-hover:border-viaduct-accent transition-colors">
                              <span className="text-sm font-bold text-viaduct-accent">{symbol.slice(0, 2)}</span>
                            </div>
                            <span className="font-medium text-white group-hover:text-viaduct-accent transition-colors">{symbol}</span>
                          </Link>
                        </td>
