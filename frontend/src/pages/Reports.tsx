...
function scoreColor(score: number | null): string {
  if (score === null) return "text-viaduct-text-secondary";
  ...
  return (
    <div className="print-header hidden">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-viaduct-accent">
            Stellar Viaduct · Network Monitor
          </p>
        ...
        <div className="text-right text-xs text-viaduct-text-secondary">
          <p>Generated</p>
...
function ReportFooter() {
  return (
    <div className="print-footer hidden text-center text-xs text-viaduct-text-secondary">
      Stellar Viaduct · Network Monitor · Confidential — for internal use
      only
    </div>
  );
}
      ...
      <section className="print-avoid-break">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={stat.label}
              className="bg-viaduct-card border border-viaduct-border rounded-lg p-4 print-avoid-break"
            >
              <p className="text-xs text-viaduct-text-secondary uppercase tracking-wide">
                {stat.label}
              </p>
      ...
      <section className="print-avoid-break">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div
              key={row.label}
              className="bg-viaduct-card border border-viaduct-border rounded-lg p-4"
            >
              <p className="text-xs text-viaduct-text-secondary">{row.label}</p>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {(assetsData ?? []).map((asset) => (
                <tr
                  key={asset.symbol}
                  className="border-b border-viaduct-border/50 text-white print-avoid-break"
                >
                  <td className="py-2 pr-4 font-mono font-semibold">
                    {asset.symbol}
                  </td>
                  <td className="py-2 pr-4 text-viaduct-text-secondary">
                    {asset.name}
                  </td>
                  ...
                  <td className="py-2 capitalize text-viaduct-text-secondary">
                    {asset.health?.trend ?? "—"}
                  </td>
                </tr>
              ))}
              {(assetsData ?? []).length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    className="py-6 text-center text-viaduct-text-secondary"
                  >
  ...
  return (
    <div className="space-y-6">
      ...
      {(assetsData ?? []).map((asset, idx) => (
        <section
          key={asset.symbol}
          className={`bg-viaduct-card border border-viaduct-border rounded-lg p-6 print-avoid-break ${
        ...
        >
          ...
          {asset.health && (
            <dl className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              {[
                ...
                <div
                  key={item.label}
                  className="bg-viaduct-background rounded-lg p-3"
                >
          ...
          <p className="mt-3 text-xs text-viaduct-text-secondary">
            Last updated:{" "}
      ...
      {(assetsData ?? []).length === 0 && (
        <p className="text-center text-viaduct-text-secondary py-8">
...
export default function Reports() {
  ...
  return (
    <div className="space-y-8">
          ...
          {TEMPLATES.map((tpl) => (
            <button
              ...
              className={[
                ...
                "focus:ring-2 focus:ring-viaduct-accent",
                activeTemplate === tpl.id
                  ? "border-viaduct-accent bg-viaduct-accent/10 text-white"
                  : "border-viaduct-border bg-viaduct-card text-viaduct-text-secondary hover:text-white",
              ].join(" ")}
            >
      ...
      <section className="no-print">
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div>
            <label
              htmlFor="date-from"
              className="block text-sm text-viaduct-text-secondary mb-1"
            >
              From
            </label>
            <input
              ...
              className="rounded-lg border border-viaduct-border bg-viaduct-card px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
            />
          </div>
          <div>
            <label
      ...
      <div
        id="report-content"
        className="bg-viaduct-card border border-viaduct-border rounded-lg p-6 space-y-4"
        aria-live="polite"
      >
        {/* Screen-only report meta banner */}
        <div className="no-print flex items-center justify-between border-b border-viaduct-border pb-4">
          <div>
            <p className="text-xs uppercase tracking-widest text-viaduct-accent font-semibold">
              Stellar Viaduct · Network Monitor
            </p>
            <h2 className="text-xl font-bold text-white mt-1">
              {TEMPLATES.find((t) => t.id === activeTemplate)?.label}
            </h2>
            <p className="text-sm text-viaduct-text-secondary mt-1">
              Period: {formatDate(dateRange.from)} – {formatDate(dateRange.to)}{" "}
        ...
        {isLoading ? (
          <div
            className="py-16 text-center text-viaduct-text-secondary"
            role="status"
          >
            Loading report data…
          </div>
