...
export default function RequestBuilder({ endpoint }: Props) {
  ...
  if (endpoint.method === "WS") {
    return (
      <p className="text-sm text-viaduct-text-secondary italic">
    ...
    <div className="space-y-4">
      ...
      {(endpoint.pathParams ?? []).length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-viaduct-text-secondary uppercase tracking-wide">Path Parameters</p>
          {endpoint.pathParams!.map((p) => (
            <div key={p.name} className="flex items-center gap-3">
              <label className="w-24 text-sm font-mono text-white shrink-0">{p.name}</label>
              <input
                className="flex-1 bg-viaduct-background border border-viaduct-border rounded px-3 py-1.5 text-sm text-white placeholder-stellar-text-secondary focus:outline-none focus:border-viaduct-accent"
      ...
      <div className="flex items-center gap-2 bg-viaduct-background border border-viaduct-border rounded px-3 py-2">
        <span className="text-xs font-mono text-viaduct-text-secondary shrink-0">URL</span>
      ...
      <button
        onClick={tryIt}
        disabled={loading}
        className="px-4 py-2 bg-viaduct-accent text-white text-sm font-medium rounded hover:bg-blue-600 disabled:opacity-50 transition-colors"
      >
