    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-viaduct-text-secondary">
          Interactive documentation for the Stellar Viaduct REST and WebSocket API.
        </p>
      </div>

      {/* Auth & Rate Limit info */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-4 space-y-1">
      ...
      {/* Base URL */}
      <div className="bg-viaduct-card border border-viaduct-border rounded-lg px-4 py-3 flex items-center gap-3">
        <span className="text-xs text-viaduct-text-secondary shrink-0">Base URL</span>
        <code className="text-sm text-white font-mono">https://api.stellarviaduct.io</code>
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-viaduct-text-secondary text-sm">No endpoints match your search.</p>
      ) : (
        CATEGORIES.filter((cat) => filtered.some((e) => e.category === cat)).map((cat) => {
          ...
          return (
            <section key={cat} className="space-y-3">
              <h2 className="text-lg font-semibold text-white border-b border-viaduct-border pb-2">{cat}</h2>
              {group.map((endpoint) => (
