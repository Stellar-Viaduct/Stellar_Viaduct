    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <mark key={i} className="bg-viaduct-accent/30 text-white rounded-sm px-0.5">
            {part}
          </mark>
        ) : (
...
function ResultCard({ result, query }: { result: IndexedSearchResult; query: string }) {
  ...
  return (
    <Link
      to={href}
      className="flex items-start gap-4 p-4 rounded-xl border border-viaduct-border bg-viaduct-card hover:border-viaduct-accent/50 hover:bg-viaduct-card/80 transition-colors group"
    >
      <span className="flex-none w-10 h-10 flex items-center justify-center rounded-lg bg-stellar-border text-viaduct-text-secondary text-base font-mono group-hover:bg-viaduct-accent/20 group-hover:text-viaduct-accent transition-colors">
        {icon}
      </span>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="flex-none text-xs px-1.5 py-0.5 rounded bg-stellar-border text-viaduct-text-secondary uppercase tracking-wide">
            {result.type}
          </span>
        </div>
        {result.description && (
          <p className="mt-1 text-sm text-viaduct-text-secondary line-clamp-2">
            <Highlight text={result.description} query={query} />
          </p>
        )}
        {result.highlights.length > 0 && (
          <p className="mt-1.5 text-xs text-viaduct-text-secondary/70 italic line-clamp-1">
            …<Highlight text={result.highlights[0]} query={query} />…
          </p>
        )}
      </div>
      <span className="flex-none text-viaduct-text-secondary opacity-0 group-hover:opacity-100 transition-opacity text-sm self-center">
        →
      </span>
    </Link>
  );
...
function EmptyState({ query }: { query: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="mt-2 text-sm text-viaduct-text-secondary max-w-sm">
...
function ResultSkeleton() {
  return (
    <div className="flex items-start gap-4 p-4 rounded-xl border border-viaduct-border bg-viaduct-card animate-pulse">
  ...
  return (
    <nav className="flex items-center justify-center gap-1 mt-8" aria-label="Pagination">
      <button
        ...
        className="px-3 py-1.5 rounded-lg text-sm text-viaduct-text-secondary hover:text-white hover:bg-stellar-border disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      >
        ← Prev
      </button>
      {pages.map((p, i) =>
        p === "…" ? (
          <span key={`ellipsis-${i}`} className="px-2 text-viaduct-text-secondary">
            …
          </span>
        ) : (
          <button
            ...
            className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${
              p === page
                ...
                : "text-viaduct-text-secondary hover:text-white hover:bg-stellar-border"
            }`}
          >
            {p}
          </button>
...
export default function SearchResultsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-viaduct-text-primary">Search Results</h1>
        {queryParam && !isFetching && (
          <p className="mt-1 text-viaduct-text-secondary text-sm">
            {total === 0
              ? "No results found"
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <div className="relative flex-1">
          <svg
            className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-viaduct-text-secondary pointer-events-none"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-viaduct-card border border-viaduct-border text-white placeholder:text-viaduct-text-secondary focus:outline-none focus:border-viaduct-accent/60 transition-colors"
            autoFocus
          />
        </div>
        <button
          type="submit"
          className="px-5 py-2.5 rounded-xl bg-viaduct-accent text-white font-medium hover:bg-viaduct-accent/90 transition-colors text-sm"
        >
          Search
        </button>
      </form>
      ...
      {queryParam.length >= 2 && (
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                    isActive
                      ? "bg-viaduct-accent text-white"
                      : "bg-stellar-border text-viaduct-text-secondary hover:text-white hover:bg-stellar-border/80"
                  }`}
                >
                  {TYPE_LABELS[t]}
                  {!isFetching && (
                    <span className={`ml-1.5 ${isActive ? "text-white/80" : "text-viaduct-text-secondary"}`}>
                      {count}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-viaduct-text-secondary">Sort:</span>
      ...
      {showSkeletons ? (
        ...
        <>
          ...
          {total > 0 && (
            <p className="text-center text-xs text-viaduct-text-secondary pb-4">
