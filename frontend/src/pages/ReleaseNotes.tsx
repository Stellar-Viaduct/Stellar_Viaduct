...
export default function ReleaseNotes() {
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold text-viaduct-text-primary mb-2">Release Notes</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-viaduct-border bg-viaduct-background pl-10 pr-4 py-2 text-sm text-viaduct-text-primary placeholder-stellar-text-secondary focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
          />
        </div>
            ...
            <div key={release.id} id={`v${release.version}`} className="relative pl-12">
              ...
              <div
                className={`absolute left-2 top-5 h-5 w-5 -translate-x-1/2 rounded-full border-2 border-stellar-dark flex items-center justify-center ${
                  release.tags.includes("breaking") ? "bg-red-500" : "bg-viaduct-accent"
                }`}
                aria-hidden="true"
              >
              ...
              <div className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-4">
                {/* Header */}
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      <h2 className="text-xl font-bold text-viaduct-text-primary">
                        v{release.version}
                      </h2>
                    ...
                    <p className="text-xs text-viaduct-text-secondary mt-1">{release.date}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => copyLink(release.version)}
                    title="Copy shareable link"
                    className="rounded-md p-1.5 text-viaduct-text-secondary hover:text-white hover:bg-viaduct-surface transition-colors"
                  >
                ...
                <p className="text-sm text-viaduct-text-secondary">{release.summary}</p>

                {/* Entries */}
                <div className="space-y-4 pt-2 border-t border-viaduct-border">
                  {release.entries
                    ...
                    .map((entry, idx) => (
                      <div key={idx} className="space-y-1">
                        ...
                        <p className="text-sm text-viaduct-text-secondary pl-1">{entry.description}</p>
                        {entry.migrationNote && (
                          <div className="mt-2 rounded-lg border border-red-500/20 bg-red-500/5 p-3">
                            ...
                            <p className="text-xs text-viaduct-text-secondary">{entry.migrationNote}</p>
                          </div>
                        )}
                      </div>
                    ))}
