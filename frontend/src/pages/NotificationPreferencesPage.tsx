...
const STORAGE_KEY = "stellar-viaduct:notification-extended-prefs:v1";
...
function Toggle({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent focus:ring-offset-2 focus:ring-offset-stellar-card ${
        checked ? "bg-viaduct-accent" : "bg-stellar-border"
      }`}
    >
      <span
...
export default function NotificationPreferencesPage() {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      <div>
        <h1 className="text-3xl font-bold text-viaduct-text-primary mb-2">
      ...
      <section className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-4">
        <h2 className="text-lg font-semibold text-viaduct-text-primary">Sound</h2>
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm text-viaduct-text-primary">Notification sounds</p>
            <p className="text-xs text-viaduct-text-secondary mt-0.5">
      ...
      {/* Delivery channels */}
      <section className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-4">
        <h2 className="text-lg font-semibold text-viaduct-text-primary">Delivery channels</h2>
        ...
        <div className="space-y-3">
          {channelConfig.map((ch) => (
            <div key={ch.id} className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm text-viaduct-text-primary">{ch.label}</p>
                <p className="text-xs text-viaduct-text-secondary mt-0.5">{ch.description}</p>
              </div>
              <Toggle
      ...
      {/* Digest frequency */}
      <section className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-4">
        <h2 className="text-lg font-semibold text-viaduct-text-primary">Digest frequency</h2>
        <p className="text-sm text-viaduct-text-secondary">
      ...
      <section className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-viaduct-text-primary">Quiet hours</h2>
            <p className="text-sm text-viaduct-text-secondary mt-0.5">
        ...
        {extended.quietHoursEnabled && (
          <div className="flex flex-wrap gap-4 pt-2">
            <label className="flex flex-col gap-1">
              <span className="text-xs text-viaduct-text-secondary">From</span>
              <input
                type="time"
                value={extended.quietHoursStart}
                onChange={(e) => update({ quietHoursStart: e.target.value })}
                className="rounded-md border border-viaduct-border bg-viaduct-background px-3 py-1.5 text-sm text-viaduct-text-primary focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
              />
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-xs text-viaduct-text-secondary">Until</span>
              <input
                type="time"
      ...
      {/* Priority overrides */}
      <section className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-4">
        <h2 className="text-lg font-semibold text-viaduct-text-primary">Priority overrides</h2>
        <p className="text-sm text-viaduct-text-secondary">
          ...
          {priorityOptions.map((opt) => (
            <label key={opt.id} className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                className="mt-0.5 h-4 w-4 border-viaduct-border bg-viaduct-background text-viaduct-accent focus:ring-viaduct-accent"
              />
              <span>
                <span className="text-sm text-viaduct-text-primary">{opt.label}</span>
                <span className="block text-xs text-viaduct-text-secondary mt-0.5">
                  {opt.description}
                </span>
              </span>
            </label>
