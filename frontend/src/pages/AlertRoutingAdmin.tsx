...
export default function AlertRoutingAdmin() {
  const [adminToken, setAdminToken] = useLocalStorageState(
    "stellar-viaduct:admin-api-key:v1",
    ""
  );
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-viaduct-accent">Admin</p>
          <h1 className="mt-2 text-3xl font-bold text-white">Alert routing</h1>
          <p className="mt-2 max-w-2xl text-viaduct-text-secondary">
        ...
        <div className="rounded-2xl border border-viaduct-border bg-viaduct-card/80 px-5 py-4">
          <p className="text-xs uppercase tracking-[0.2em] text-viaduct-text-secondary">
            Active rules
          </p>
          <p className="mt-2 text-3xl font-semibold text-white">{activeRules}</p>
          <p className="mt-1 text-sm text-viaduct-text-secondary">Total rules: {rules.length}</p>
        </div>
      </header>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr,0.85fr]">
        <form
          ...
          className="rounded-3xl border border-viaduct-border bg-viaduct-card/80 p-6 space-y-6"
        >
          <div>
          ...
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-viaduct-border bg-viaduct-background px-4 py-3 text-white outline-none transition focus:border-viaduct-accent focus:ring-2 focus:ring-viaduct-accent"
            />
          </label>
                  ...
                  <button
                    ...
                    className={`rounded-2xl border px-3 py-2 text-sm transition ${
                      checked
                        ...
                        : "border-viaduct-border bg-viaduct-background text-viaduct-text-secondary"
                    }`}
                  >
                    {severity}
                  </button>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <label className="flex items-center justify-between rounded-2xl border border-viaduct-border bg-viaduct-background px-4 py-3 mt-6 sm:mt-0">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              disabled={loading}
              className="rounded-2xl bg-viaduct-accent px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              className="rounded-2xl border border-viaduct-border px-5 py-3 text-sm font-semibold text-viaduct-text-secondary transition hover:border-viaduct-accent hover:text-white"
            >
              Refresh
            </button>
          </div>
        ...
        <section className="space-y-6">
          <div className="rounded-3xl border border-viaduct-border bg-viaduct-card/80 p-6">
            <div className="flex items-center justify-between gap-3">
              ...
              <input
                ...
                className="w-44 rounded-xl border border-viaduct-border bg-viaduct-background px-3 py-2 text-xs text-white outline-none transition focus:border-viaduct-accent focus:ring-2 focus:ring-viaduct-accent"
              />
            </div>

            <div className="mt-4 space-y-3">
              {rules.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-viaduct-border px-4 py-8 text-center text-sm text-viaduct-text-secondary">
                  No routing rules found.
                </div>
              ) : (
                rules.map((rule) => (
                  <article key={rule.id} className="rounded-2xl border border-viaduct-border bg-viaduct-background/70 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-sm font-semibold text-white">{rule.name}</h3>
                        <p className="mt-1 text-xs text-viaduct-text-secondary">
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        ...
                        className="rounded-full border border-viaduct-border px-3 py-1 text-xs text-viaduct-text-secondary transition hover:border-viaduct-accent hover:text-white"
                      >
          ...
          <div className="rounded-3xl border border-viaduct-border bg-viaduct-card/80 p-6">
            ...
            <div className="mt-4 max-h-[24rem] space-y-2 overflow-y-auto pr-1">
              {audit.length === 0 ? (
                ...
                audit.map((entry) => (
                  <article key={entry.id} className="rounded-2xl border border-viaduct-border bg-viaduct-background/70 p-3">
                    <div className="flex items-center justify-between gap-3 text-xs">
                      <span className="font-medium text-white">
                        {entry.assetCode} • {entry.sourceType}
                      </span>
                      <span className="text-viaduct-text-secondary">{entry.status}</span>
                    </div>
                    <p className="mt-1 text-[11px] text-viaduct-text-secondary">
