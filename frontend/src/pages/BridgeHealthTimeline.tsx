    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px] max-w-xs">
          <label
            ...
            className="block text-sm font-medium text-viaduct-text-secondary mb-1"
          >
            Bridge
          </label>
          <select
            ...
            className="w-full rounded-lg border border-viaduct-border bg-viaduct-card px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-viaduct-accent disabled:opacity-50"
          >
            {bridgesLoading && (
        ...
        <div>
          <p className="text-sm font-medium text-viaduct-text-secondary mb-1">Time Period</p>
          <div className="flex gap-1" role="group" aria-label="Time period">
            {PERIODS.map((p) => (
              <button
                ...
                className={`rounded px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent ${
                  period === p.id
      ...
      {points.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            >
              <p className="text-xs text-viaduct-text-secondary uppercase tracking-wide">{stat.label}</p>
              <p
              ...
              {stat.sub && (
                <p className="text-xs text-viaduct-text-secondary mt-0.5">{stat.sub}</p>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {isMockData && (
            <span className="text-xs text-stellar-text-muted border border-viaduct-border rounded px-2 py-1">
              Demo data
            </span>
          )}
        </div>
        ...
        {/* Legend */}
        <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-viaduct-border text-xs text-viaduct-text-secondary">
