    ...
    return (
      <div className="space-y-3 text-sm">
        ...
        <dl className="space-y-2">
          <div>
            <dt className="text-viaduct-text-secondary">Chain ID</dt>
          ...
          <div>
            <dt className="text-viaduct-text-secondary">Total supply (USD)</dt>
            <dd className="text-white">${node.totalSupplyUsd.toLocaleString()}</dd>
          </div>
        </dl>
        <div>
          <p className="text-viaduct-text-secondary">Assets</p>
  ...
  if (edge) {
    return (
      <div className="space-y-3 text-sm">
        ...
        <dl className="space-y-2">
          <div>
            <dt className="text-viaduct-text-secondary">Route</dt>
            <dd className="text-white">
              {edge.source} → {edge.target}
            </dd>
          </div>
          <div>
            <dt className="text-viaduct-text-secondary">Status</dt>
            <dd className="text-white">{edge.status}</dd>
          </div>
          <div>
            <dt className="text-viaduct-text-secondary">24h volume (USD)</dt>
            <dd className="text-white">${edge.volume24hUsd.toLocaleString()}</dd>
          </div>
          <div>
            <dt className="text-viaduct-text-secondary">Assets</dt>
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-viaduct-text-primary">Bridge Topology Explorer</h1>
        ...
        <a
          ...
          className="text-sm text-viaduct-accent hover:underline"
        >
          Topology model docs
        </a>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <label className="text-sm text-viaduct-text-secondary">
          Filter chains
          <input
            className="ml-2 rounded-md border border-viaduct-border bg-viaduct-background px-3 py-1.5 text-white"
            value={chainFilter}
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[1fr_300px]">
        <div className="min-h-[480px] rounded-xl overflow-hidden border border-viaduct-border">
          <SupplyChainViz data={filteredGraph} isLoading={isLoading} error={error?.message ?? null} />
        </div>
        <aside className="rounded-xl border border-viaduct-border bg-viaduct-card p-4">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-viaduct-text-secondary">
            Drill-down
          </h2>
