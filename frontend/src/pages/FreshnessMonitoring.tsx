...
function StatusBadge({ status }: { status: "fresh" | "stale" | "unknown" }) {
  ...
  return (
    <span className="inline-flex items-center rounded-full bg-stellar-border px-2.5 py-0.5 text-xs font-medium text-viaduct-text-secondary">
      Unknown
    </span>
  );
}
...
function TrendIcon({ trend }: { trend?: string | null }) {
  ...
  return <span className="text-viaduct-text-secondary" aria-label="Stable">→</span>;
}
...
export default function FreshnessMonitoring() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-viaduct-text-primary">Data Freshness</h1>
      ...
      {!isLoading && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="rounded-lg border border-viaduct-border bg-viaduct-card p-4">
            <p className="text-sm text-viaduct-text-secondary">Total Sources</p>
          ...
          <div className="rounded-lg border border-green-800/40 bg-green-900/10 p-4">
            <p className="text-sm text-viaduct-text-secondary">Fresh</p>
          ...
          <div className="rounded-lg border border-amber-800/40 bg-amber-900/10 p-4">
            <p className="text-sm text-viaduct-text-secondary">Stale</p>
        ...
        <div className="rounded-lg border border-amber-800/40 bg-amber-900/10 p-4">
          ...
          <ul className="space-y-1">
            {alertsData.alerts.map((alert, i) => (
              <li key={i} className="text-sm text-viaduct-text-secondary">
      ...
      <div className="rounded-lg border border-viaduct-border bg-viaduct-card">
        <div className="px-6 py-4 border-b border-viaduct-border">
          <h2 className="text-lg font-semibold text-viaduct-text-primary">Source Freshness Status</h2>
        </div>
                ...
                {sources.map((source) => (
                  <tr
                    ...
                    className={
                      source.status === "stale"
                        ? "bg-amber-900/10"
                        : "hover:bg-viaduct-surface/40"
                    }
                  >
                    <td className="px-6 py-4">
                      <div className="font-medium text-viaduct-text-primary">{source.label}</div>
                      <div className="text-xs text-viaduct-text-secondary">{source.key}</div>
                    </td>
                    <td className="px-6 py-4">
                      <StatusBadge status={source.status} />
                    </td>
                    <td className="px-6 py-4 text-viaduct-text-secondary">
