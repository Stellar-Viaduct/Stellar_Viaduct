    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          ...
          className={`rounded-lg px-5 py-2.5 text-sm font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent ${
            isOpen
              ? "border border-viaduct-border text-viaduct-text-secondary hover:text-white"
              : "bg-viaduct-accent text-white hover:bg-viaduct-accent/90"
          }`}
          aria-pressed={isOpen}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {pinned.length > 0 && (
              <button
                ...
                className="text-xs text-viaduct-text-secondary hover:text-red-400 transition-colors"
              >
                Unpin all
              </button>
            )}
          ...
          {sortedPinned.length === 0 ? (
            <div className="rounded-lg border border-viaduct-border border-dashed p-8 text-center">
              <svg
              ...
              <p className="text-viaduct-text-secondary text-sm mt-1">
      ...
      <section className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
        ...
        <div className="grid sm:grid-cols-3 gap-4 text-sm text-viaduct-text-secondary">
          {[
            {
              title: "Persistent",
            ...
            <div key={item.title} className="flex gap-3">
              <div className="flex-shrink-0 mt-0.5">
                <span className="w-5 h-5 rounded-full bg-viaduct-accent/20 text-viaduct-accent flex items-center justify-center text-xs font-bold">
                  ✓
                </span>
              </div>
              <div>
          ...
          {AVAILABLE_METRICS.map((m) => {
            ...
            return (
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent ${
                  isPinned
                    ? "bg-viaduct-accent text-white"
                    : "border border-viaduct-border text-viaduct-text-secondary hover:text-white hover:border-viaduct-accent/50"
                }`}
              >
                {m.label}
              </button>
