...
export default function LiquidityDashboard() {
  const [pair, setPair] = useLocalStorageState<TradingPair>(
    "stellar-viaduct:liquidity-pair:v1",
    "USDC/XLM"
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          >
            <p className="text-xs text-viaduct-text-secondary">{stat.label}</p>
        ...
        >
          ...
          {venues.length > 0 && (
            <table className="mt-4 w-full text-xs" aria-label="Venue liquidity breakdown">
              <thead>
                <tr className="text-viaduct-text-secondary border-b border-viaduct-border">
              ...
              <tbody>
                {venues.map((v: VenueLiquidity) => (
                  <tr key={v.venue} className="border-b border-viaduct-border/50">
                    <td className="py-2 text-white">{v.venue}</td>
                    <td className="py-2 text-right text-viaduct-text-secondary">
