...
export default function CustomMetricBuilder() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-viaduct-text-primary">Custom Metric Builder</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <section className="space-y-4 rounded-xl border border-viaduct-border bg-viaduct-card p-6">
          <div>
            <label htmlFor="metric-name" className="text-sm text-viaduct-text-secondary">
              Metric name
            </label>
            <input
              id="metric-name"
              className="mt-1 w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-white"
              value={name}
          ...
          <div>
            <label htmlFor="metric-description" className="text-sm text-viaduct-text-secondary">
              Description
            </label>
            <input
          ...
          <div>
            <label htmlFor="metric-formula" className="text-sm text-viaduct-text-secondary">
              Formula (SQL SELECT)
            </label>
            <textarea
              id="metric-formula"
              className="mt-1 h-48 w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 font-mono text-sm text-white"
              value={formula}
          ...
          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              className="rounded-md border border-viaduct-border px-4 py-2 text-sm text-white hover:bg-viaduct-surface"
            ...
            <button
              type="button"
              className="rounded-md bg-viaduct-accent px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
              onClick={handleSave}
              disabled={!canSave}
            >
              Save metric
          ...
          {validationMessage && (
            <p className="text-sm text-viaduct-text-secondary">{validationMessage}</p>
          )}

          {preview && (
            <div className="rounded-md border border-viaduct-border bg-viaduct-background p-4">
              <p className="text-sm font-medium text-white">Preview ({preview.rowCount} rows)</p>
              <pre className="mt-2 overflow-auto text-xs text-viaduct-text-secondary">
        ...
        <aside className="rounded-xl border border-viaduct-border bg-viaduct-card p-6">
          ...
          {isLoading ? (
            <p className="mt-4 text-sm text-viaduct-text-secondary">Loading...</p>
          ) : metrics.length === 0 ? (
            <p className="mt-4 text-sm text-viaduct-text-secondary">No saved metrics yet.</p>
          ) : (
            <ul className="mt-4 space-y-3">
              {metrics.map((metric) => (
                <li
                  key={metric.id}
                  className="rounded-md border border-viaduct-border bg-viaduct-background p-3"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-medium text-white">{metric.name}</p>
                      {metric.description && (
                        <p className="text-xs text-viaduct-text-secondary">{metric.description}</p>
                      )}
                      {metric.isShared && (
                        <span className="mt-1 inline-block text-xs text-viaduct-accent">Shared</span>
                      )}
                    </div>
                    <button
                      type="button"
