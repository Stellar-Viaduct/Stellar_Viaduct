...
export default function AssetDetail() {
  ...
  if (!symbol) {
    return <div className="text-viaduct-text-secondary">No asset symbol provided.</div>;
  }
  ...
  return (
    <div className="space-y-8">
        ...
        actions={
          <>
            ...
            <button
              ...
              className="rounded-full border border-viaduct-border px-4 py-1.5 text-xs text-white hover:bg-stellar-border"
            >
              Refresh views
            </button>
      ...
      <Tabs activeTab={activeTab} onTabChange={handleTabChange}>
        <TabList aria-label="Asset detail views" className="flex flex-wrap items-center gap-2 border-b border-viaduct-border pb-4">
            ...
            <div className="space-y-3 lg:col-span-2">
              ...
              <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-4">
                {priceData?.history && priceData.history.length > 0 ? (
                  ...
                  <p className="text-viaduct-text-secondary">Price data not available</p>
                )}
              </div>
            </div>
          </div>
                    ...
                    {priceData?.sources && priceData.sources.length > 0 ? (
                      priceData.sources.map(
                        (source: {
                          ...
                          <tr key={source.source} className="border-b border-viaduct-border">
                            <td className="py-3 pr-4">{source.source}</td>
                            <td className="py-3 pr-4">${source.price.toFixed(4)}</td>
                            <td className="py-3 pr-4 text-viaduct-text-secondary">{source.timestamp}</td>
                      ...
                      <tr>
                        <td colSpan={4} className="py-6 text-center text-viaduct-text-secondary">
                          No price source data available
                        </td>
                      </tr>
                    )}
