  ...
  return (
    <button
      ...
      className={`rounded-lg border px-3 py-2 text-sm transition-colors text-left ${
        active
          ? "border-viaduct-accent bg-viaduct-accent/10 text-white"
          : "border-viaduct-border bg-viaduct-card text-viaduct-text-secondary hover:text-white hover:border-viaduct-accent/50"
      }`}
    >
      <p className="font-medium capitalize">{label}</p>
      <p className="text-xs text-viaduct-text-secondary mt-0.5">{item.nodeCount} nodes</p>
    </button>
  );
}
...
function NodeDrawer({
  ...
  return (
    <aside
      className="w-72 shrink-0 rounded-xl border border-viaduct-border bg-viaduct-card p-5 space-y-4 overflow-y-auto"
      aria-label="Node detail drawer"
    >
      <div className="flex items-start justify-between gap-2">
        <h2 className="text-base font-semibold text-viaduct-text-primary leading-tight">
          {node.label}
        </h2>
        <button
          type="button"
          onClick={onClose}
          className="text-viaduct-text-secondary hover:text-white text-lg leading-none shrink-0"
      ...
      <div className="text-xs text-viaduct-text-secondary space-y-1.5">
        <p>
          <span className="text-viaduct-text-primary font-medium">ID: </span>
          <span className="font-mono">{node.id}</span>
        </p>
        <p>
          <span className="text-viaduct-text-primary font-medium">Entity: </span>
          {node.entityType} · {node.entityId}
        </p>
        <p>
          <span className="text-viaduct-text-primary font-medium">Last updated: </span>
          {new Date(node.timestamp).toLocaleString()}
        </p>
      </div>

      {node.description && (
        <p className="text-sm text-viaduct-text-secondary">{node.description}</p>
      )}

      {Object.keys(node.metadata).length > 0 && (
        <div className="rounded-lg border border-viaduct-border bg-viaduct-background p-3">
          <p className="text-xs font-medium text-viaduct-text-primary mb-2">Metadata</p>
          <dl className="space-y-1">
            {Object.entries(node.metadata).map(([k, v]) => (
              <div key={k} className="flex gap-2 text-xs">
                <dt className="text-viaduct-text-secondary shrink-0">{k}</dt>
                <dd className="text-viaduct-text-primary font-mono truncate">{String(v)}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      {connected.length > 0 && (
        <div>
          <p className="text-xs font-medium text-viaduct-text-primary mb-2">
              ...
              return (
                <li key={i}>
                  <button
                    ...
                    className="w-full text-left rounded-md border border-viaduct-border bg-viaduct-background px-3 py-1.5 text-xs hover:bg-stellar-border/40 transition-colors flex items-center justify-between gap-2"
                  >
                    <span className="flex items-center gap-1 truncate">
                      <span className="text-viaduct-text-secondary">{isOut ? "→" : "←"}</span>
                      <span className="text-viaduct-text-primary truncate">
                        {other?.label ?? otherId}
                      </span>
                    </span>
                    <span className="text-viaduct-text-secondary shrink-0 font-mono">
                      {e.transformKind}
                    </span>
                  </button>
                </li>
...
export default function DataProvenanceGraph() {
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
      <div>
        <h1 className="text-3xl font-bold text-viaduct-text-primary mb-1">
          Data Provenance Graph
        </h1>
        <p className="text-viaduct-text-secondary text-sm">
      ...
      <section aria-label="Metric filters and selector">
        <div className="flex flex-wrap gap-3 items-center mb-4">
          <input
            ...
            className="rounded-lg border border-viaduct-border bg-viaduct-background px-3 py-1.5 text-sm text-viaduct-text-primary placeholder-stellar-text-secondary focus:outline-none focus:ring-2 focus:ring-viaduct-accent w-40"
          />
          <input
            type="text"
          ...
          <select
            ...
            className="rounded-lg border border-viaduct-border bg-viaduct-background px-3 py-1.5 text-sm text-viaduct-text-primary focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
          >
        ...
        {metricsLoading && (
          <p className="text-sm text-viaduct-text-secondary">Loading metrics…</p>
        )}
        ...
        {!metricsLoading && metrics.length === 0 && (
          <p className="text-sm text-viaduct-text-secondary">No metrics match the current filters.</p>
        )}
      </section>
      ...
      {selectedMetric && (
        <>
          ...
          {graphData && (
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <div
                  key={s.label}
                  className="rounded-lg border border-viaduct-border bg-viaduct-card px-4 py-3 min-w-[100px]"
                >
                  <p className="text-xs text-viaduct-text-secondary">{s.label}</p>
                  <p className={`text-2xl font-bold ${s.color || "text-viaduct-text-primary"}`}>
                    {s.value}
                  </p>
                </div>
              ))}
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                {selectedNodeId && (
                  <button
                    ...
                    className="rounded-lg border border-viaduct-border bg-viaduct-background px-3 py-1.5 text-sm text-viaduct-text-secondary hover:text-white transition-colors"
                  >
                    Clear selection
                  </button>
                )}
          ...
          {/* Kind legend */}
          <div className="flex gap-4 flex-wrap text-xs text-viaduct-text-secondary items-center">
          ...
          <div className="flex gap-4 flex-1 min-h-0" style={{ minHeight: 400 }}>
            {/* Graph canvas */}
            <div className="flex-1 rounded-xl border border-viaduct-border bg-viaduct-card overflow-hidden relative">
              {graphLoading && (
                <div className="absolute inset-0 flex items-center justify-center text-viaduct-text-secondary text-sm">
          ...
          {/* Node list table */}
          {graphData && (
            <details className="rounded-xl border border-viaduct-border bg-viaduct-card">
              <summary className="px-5 py-3 text-sm font-medium text-viaduct-text-primary cursor-pointer select-none">
                Node list ({filteredNodes.length})
              </summary>
              <div className="px-5 pb-4 overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-viaduct-border text-left">
                      {["Label", "Kind", "Entity", "Freshness", "Last updated"].map((h) => (
                        <th key={h} className="py-2 pr-4 text-xs text-viaduct-text-secondary font-medium">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {filteredNodes.map((n) => (
                      <tr
                        key={n.id}
                        className={`border-b border-viaduct-border/40 cursor-pointer hover:bg-viaduct-surface/50 transition-colors ${
                          selectedNodeId === n.id ? "bg-viaduct-accent/10" : ""
                        }`}
                        onClick={() =>
                          setSelectedNodeId(selectedNodeId === n.id ? null : n.id)
                        }
                      >
                        <td className="py-2 pr-4 text-viaduct-text-primary font-medium">{n.label}</td>
                        ...
                        <td className="py-2 pr-4 text-viaduct-text-secondary text-xs">
      ...
      {!selectedMetric && !metricsLoading && (
        <div className="flex-1 flex items-center justify-center rounded-xl border border-viaduct-border bg-viaduct-card text-viaduct-text-secondary text-sm">
