...
describe("viaduct:open-search custom event", () => {
  it("dispatches and is received by event listener", () => {
    const listener = vi.fn();
    window.addEventListener("viaduct:open-search", listener);
    window.dispatchEvent(new CustomEvent("viaduct:open-search"));
    expect(listener).toHaveBeenCalledOnce();
    window.removeEventListener("viaduct:open-search", listener);
  });
});

describe("viaduct:open-shortcuts custom event", () => {
  it("dispatches and is received by event listener", () => {
    const listener = vi.fn();
    window.addEventListener("viaduct:open-shortcuts", listener);
    window.dispatchEvent(new CustomEvent("viaduct:open-shortcuts"));
    expect(listener).toHaveBeenCalledOnce();
    window.removeEventListener("viaduct:open-shortcuts", listener);
  });
});
