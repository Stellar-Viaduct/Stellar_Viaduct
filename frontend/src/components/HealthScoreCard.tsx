...
function HealthScoreCardSkeleton({ symbol }: { symbol: string }) {
  return (
    <div
      className="bg-viaduct-card border border-viaduct-border rounded-lg p-6"
      role="article"
...
export default function HealthScoreCard({
  ...
  return (
    <div
      className="bg-viaduct-card border border-viaduct-border rounded-lg p-6 hover:border-viaduct-accent transition-colors"
      role="article"
    ...
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-viaduct-text-primary truncate">{symbol}</h3>
          {name && (
            <p className="text-sm text-viaduct-text-secondary truncate">{name}</p>
          )}
        </div>
        <div
      ...
      <div className="flex justify-center mb-4">
        <div className="relative w-28 h-28">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span
              className="text-3xl font-bold text-viaduct-text-primary"
              aria-hidden="true"
            >
              {overallScore}
            </span>
          ...
          {(Object.entries(factors) as [keyof HealthFactors, number][]).map(
            ([key, value]) => {
              ...
              return (
                ...
                >
                  <span className="text-sm text-viaduct-text-secondary flex-shrink-0">
                    {FACTOR_LABELS[key]}
                  </span>
                  <div className="flex items-center gap-2 flex-1 justify-end">
                    ...
                    <span className="text-sm text-viaduct-text-primary w-7 text-right tabular-nums">
                      {value}
                    </span>
                  </div>
                </div>
