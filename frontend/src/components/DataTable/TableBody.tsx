...
function RowActionsMenu<TData>({
  ...
  return (
    ...
    >
      <button
        type="button"
        className="px-2 py-1 rounded border border-viaduct-border text-white"
        aria-haspopup="menu"
        aria-expanded={open}
      ...
      {open ? (
        <div
          role="menu"
          className="absolute right-0 mt-2 min-w-40 bg-viaduct-card border border-viaduct-border rounded shadow-lg z-20"
        >
          {(rowActions.items ?? []).map((item) => (
            <button
              ...
              type="button"
              role="menuitem"
              className="w-full text-left px-3 py-2 text-sm text-white hover:bg-viaduct-surface disabled:opacity-40"
        ...
        <tbody className="text-viaduct-text-primary">
          {isLoading ? (
            <tr>
              <td
                colSpan={colSpan}
                className="py-10 text-center text-viaduct-text-secondary"
              >
                Loading…
              </td>
            </tr>
            ...
            <>
              ...
              {visibleRows.map(({ row, idx }) => (
                <tr
                  key={row.id}
                  className={`border-b border-viaduct-border ${
                    idx === focusedRowIndex ? "bg-viaduct-background/40" : ""
                  }`}
