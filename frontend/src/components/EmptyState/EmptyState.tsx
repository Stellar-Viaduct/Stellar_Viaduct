...
function ActionButton({ action, index }: { action: EmptyStateAction; index: number }) {
  ...
  const base =
    "inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent focus:ring-offset-2 focus:ring-offset-stellar-dark";

  const primary =
    "bg-viaduct-accent text-white hover:bg-blue-600 active:bg-blue-700";

  const secondary =
    "border border-viaduct-border text-viaduct-text-secondary hover:border-viaduct-accent hover:text-white";
 ...
 * EmptyState
 *
 * Standardised empty-state component for Stellar Viaduct. Renders an optional
...
export function EmptyState({
  ...
  return (
    ...
    >
      {illustration && (
        <div
          className={`mb-4 text-viaduct-text-secondary ${illustrationSize[variant]}`}
          aria-hidden="true"
        >
          {illustration}
        </div>
      ...
      <h2 className={`text-viaduct-text-primary ${titleSize[variant]} mb-2`}>
        {title}
      </h2>

      {description && (
        <p
          className={`text-viaduct-text-secondary max-w-sm leading-relaxed ${
