      ...
      <div className="mt-4 flex items-center justify-center gap-3">
        ...
        {severity === "critical" && (
          <button
            ...
            className="px-4 py-2 text-viaduct-text-secondary border border-viaduct-border rounded-md hover:bg-stellar-border focus:outline-none focus:ring-2 focus:ring-viaduct-accent focus:ring-offset-2 focus:ring-offset-stellar-dark"
          >
            Reload Page
          </button>
        )}
      ...
      {isDev && error.stack && (
        <div className="mt-4 text-left">
          <button
            ...
            className="text-xs text-viaduct-text-secondary hover:text-viaduct-text-primary underline focus:outline-none"
            aria-expanded={showStack}
          >
            {showStack ? "Hide" : "Show"} stack trace
          </button>
          {showStack && (
            <pre className="mt-2 p-3 bg-viaduct-background/80 border border-viaduct-border rounded text-xs text-red-300 overflow-x-auto max-h-48 overflow-y-auto whitespace-pre-wrap break-words">
              {error.stack}
