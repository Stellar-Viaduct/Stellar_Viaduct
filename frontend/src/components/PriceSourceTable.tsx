...
function getStatusStyle(status: PriceSource["status"]): string {
  switch (status) {
    ...
    default:
      return "bg-stellar-border text-viaduct-text-secondary";
  }
}
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {vwap !== null && (
          <div className="text-right">
            <span className="text-xs text-viaduct-text-secondary">VWAP</span>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="text-white">
              {sources.map((source) => (
                <tr
                  key={source.source}
                  className="border-b border-viaduct-border last:border-b-0"
                >
                  ...
                  <td
                    className="py-3 pr-4 text-viaduct-text-secondary"
