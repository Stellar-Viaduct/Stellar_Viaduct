...
function SortableAssetItem({ id, onRemove }: SortableItemProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center justify-between p-3 mb-2 rounded bg-stellar-border shadow-sm border border-transparent hover:border-viaduct-accent transition-colors ${
        isDragging ? "ring-2 ring-viaduct-accent" : ""
      }`}
    >
      <div className="flex items-center gap-3">
        ...
        <Link to={`/assets/${id}`} className="font-bold text-white hover:text-viaduct-accent transition-colors">
          {id}
        </Link>
      </div>
  ...
  return (
    <>
      <div 
        ...
        aria-hidden="true"
      />
      <div className="fixed inset-y-0 right-0 z-50 w-80 bg-viaduct-card border-l border-viaduct-border shadow-2xl flex flex-col transform transition-transform duration-300">
        ...
        <div className="p-4 border-t border-viaduct-border bg-viaduct-background">
          <Link
            to="/watchlist"
            onClick={onClose}
            className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-viaduct-accent hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-viaduct-accent"
          >
            Manage Watchlists
          </Link>
        </div>
