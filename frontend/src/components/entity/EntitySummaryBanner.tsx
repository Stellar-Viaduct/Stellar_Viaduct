...
const trendClasses: Record<SummaryTrendDirection, string> = {
  ...
  neutral: "border-viaduct-border bg-viaduct-background/50 text-viaduct-text-secondary",
};
...
function FieldSkeleton({ expanded }: { readonly expanded: boolean }) {
  return (
    <div className="rounded-lg border border-viaduct-border bg-viaduct-background/40 p-3">
...
function SummaryFieldCard({
  ...
  return (
    <div className="rounded-lg border border-viaduct-border bg-viaduct-background/40 p-3">
      <p className="text-xs font-medium uppercase tracking-wide text-viaduct-text-secondary">
        {field.label}
      </p>
      ...
      {expanded && field.hint ? (
        <p className="mt-2 text-xs text-viaduct-text-secondary">{field.hint}</p>
      ) : null}

      {field.to ? (
        <Link
          to={field.to}
          className="mt-2 inline-flex items-center text-xs font-medium text-viaduct-accent hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
        >
          {drilldownLabel} →
        </Link>
      ) : field.onDrilldown ? (
...
export default function EntitySummaryBanner({
  ...
  return (
    <section
      ...
      className={`rounded-2xl border border-viaduct-border bg-gradient-to-br from-stellar-card via-stellar-card to-stellar-dark/40 p-5 ${className}`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <span className="inline-block rounded-full border border-viaduct-border px-2 py-0.5 text-[11px] uppercase tracking-wide text-viaduct-text-secondary">
              {entityType}
            </span>
            <h2 className="mt-1 text-2xl font-bold text-white">{title}</h2>
            {subtitle ? (
              <p className="mt-1 text-sm text-viaduct-text-secondary">{subtitle}</p>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-full border border-viaduct-border px-3 py-1.5 text-xs text-viaduct-text-secondary transition-colors hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
          >
