...
export default function AlertSuppressionControls() {
  ...
  return (
    <section className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-6" aria-labelledby="suppression-controls-heading">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 id="suppression-controls-heading" className="flex items-center gap-2 text-xl font-semibold text-viaduct-text-primary">
        ...
        <button
          ...
          className="rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-viaduct-text-primary hover:bg-stellar-border/40"
        >
          Refresh
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {rules.length === 0 ? (
            <p className="text-sm text-viaduct-text-secondary">No suppression rules configured.</p>
          ) : (
            rules.map((rule) => (
              <div key={rule.id} className="rounded-lg border border-viaduct-border bg-viaduct-background p-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm text-viaduct-text-primary font-medium">{rule.name}</p>
                ...
                <button
                  ...
                  className={`rounded px-3 py-1 text-xs font-medium ${
                    rule.isActive
                      ...
                      : "bg-stellar-border/40 text-viaduct-text-secondary border border-viaduct-border"
                  }`}
                >
      ...
      <form onSubmit={handleOverrideSubmit} className="space-y-3 rounded-lg border border-viaduct-border p-4">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-viaduct-text-primary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="text-xs text-viaduct-text-secondary">
            ...
            <input
              ...
              className="mt-1 w-full rounded border border-viaduct-border bg-viaduct-background px-2 py-2 text-sm text-viaduct-text-primary"
            />
          </label>
          <label className="text-xs text-viaduct-text-secondary">
            <span className="flex items-center gap-1.5 mb-1">
        ...
        <button type="submit" className="rounded-md bg-viaduct-accent px-3 py-2 text-sm font-medium text-white">
          Add override
        </button>
      </form>

      <form onSubmit={handlePreviewSubmit} className="space-y-3 rounded-lg border border-viaduct-border p-4">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-viaduct-text-primary">
          Preview suppression
          <HelpIcon
        ...
        <button type="submit" className="rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-viaduct-text-primary">
          Preview
        </button>
        {previewResult && <p className="text-sm text-viaduct-text-secondary">Result: {previewResult}</p>}
      </form>
    </section>
  );
}
