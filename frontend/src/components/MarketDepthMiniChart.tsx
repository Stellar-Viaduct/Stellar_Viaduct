...
function DepthTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div
      className="bg-viaduct-card border border-viaduct-border rounded px-3 py-2 text-xs shadow-lg"
      role="tooltip"
    >
...
export default function MarketDepthMiniChart({
  ...
  return (
    <div className={`bg-viaduct-card border border-viaduct-border rounded-lg p-4 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
        ...
        {showAssetSelector && (
          <select
            ...
            className="bg-viaduct-card border border-viaduct-border rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-viaduct-accent"
      ...
      {data && data.midPrice > 0 && (
        <div className="flex justify-center mb-2">
          <span className="text-xs bg-stellar-border/50 rounded px-2 py-0.5 text-viaduct-text-secondary">
