...
function DetailRow({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between gap-1 py-2.5 border-b border-viaduct-border last:border-b-0">
...
export default function TransactionDetail({
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-viaduct-card border border-viaduct-border rounded-xl shadow-2xl">
        {/* Header */}
        <div className="sticky top-0 bg-viaduct-card border-b border-viaduct-border px-6 py-4 flex items-center justify-between rounded-t-xl">
          ...
          <button
            type="button"
            onClick={onClose}
            className="text-viaduct-text-secondary hover:text-white transition-colors p-1"
            aria-label="Close"
          >
            <svg
        ...
        <div className="px-6 py-4 space-y-1">
          ...
          <DetailRow label="Transaction Hash">
            <span className="inline-flex items-center gap-2 font-mono text-viaduct-accent text-xs">
              <span>{tx.txHash}</span>
              <CopyButton
                value={tx.txHash}
                label="Copy"
        ...
        {/* Explorer links */}
        <div className="px-6 py-4 border-t border-viaduct-border flex flex-wrap gap-3">
          {tx.stellarTxHash && (
            <a
              href={getStellarExplorerUrl(tx.stellarTxHash)}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 bg-viaduct-accent/10 text-viaduct-accent border border-viaduct-accent/30 rounded-lg px-4 py-2 text-sm hover:bg-viaduct-accent/20 transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4"
          ...
          {tx.stellarTxHash && (
            <CopyButton
              ...
              failedLabel="Failed"
              format="url"
              className="bg-viaduct-accent/10 text-viaduct-accent border-viaduct-accent/30 hover:bg-viaduct-accent/20 hover:text-viaduct-accent"
