      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {filtered.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className="flex items-center justify-between rounded-lg border border-viaduct-border bg-viaduct-background px-3 py-2.5"
            >
              ...
              <button
                ...
                className={`ml-2 flex-shrink-0 rounded px-2.5 py-1 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent ${
                  isPinned
                    ? "border border-viaduct-border text-viaduct-text-secondary hover:text-red-400"
                    : "bg-viaduct-accent/20 border border-viaduct-accent/40 text-viaduct-accent hover:bg-viaduct-accent/30"
                }`}
