...
export default function MetricsSidebar() {
  ...
  return (
    <aside
      className={`fixed right-0 top-16 bottom-0 z-30 flex flex-col border-l border-viaduct-border bg-viaduct-card transition-all duration-300 shadow-2xl ${
    ...
    >
      ...
      <button
        ...
        className="flex items-center justify-center h-10 border-b border-viaduct-border text-viaduct-text-secondary hover:text-white transition-colors focus:outline-none"
      ...
      {!isCollapsed && (
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          <p className="text-xs font-semibold uppercase tracking-widest text-viaduct-text-secondary px-1 pb-1">
            Pinned Metrics
          </p>
          ...
          <p className="text-xs text-stellar-text-muted text-center pt-2">
            ...
            <a href="/metrics-sidebar" className="text-viaduct-accent hover:underline">
              Metrics
            </a>{" "}
            to manage
          </p>
      ...
      {isCollapsed && (
        <div className="flex-1 flex flex-col items-center gap-3 pt-3">
          {sortedPinned.slice(0, 5).map((_, i) => (
            <div key={i} className="w-2 h-2 rounded-full bg-viaduct-accent/60" />
          ))}
