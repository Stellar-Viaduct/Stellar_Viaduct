...
export default function PinnedMetricCard({ metric, liveValue, onUnpin }: Props) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-start gap-2 rounded-lg border border-viaduct-border bg-viaduct-background p-3 group select-none ${
        isDragging ? "shadow-xl ring-1 ring-viaduct-accent/50" : ""
      }`}
    >
      {/* Drag handle */}
      <button
        type="button"
        className="flex-shrink-0 mt-0.5 cursor-grab active:cursor-grabbing text-stellar-text-muted hover:text-viaduct-text-secondary p-0.5 rounded transition-colors focus:outline-none"
        {...attributes}
        {...listeners}
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-viaduct-text-secondary truncate">{metric.label}</p>
