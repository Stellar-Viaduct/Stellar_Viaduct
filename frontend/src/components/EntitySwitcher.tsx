...
const RECENTS_KEY = "stellar-viaduct:entity-switcher-recents";
const MAX_RECENTS = 6;
  ...
  return (
    <div className="relative" ref={panelRef} onKeyDown={handleKeyDown}>
      <button
        ...
        className="inline-flex min-h-10 items-center gap-2 rounded-md border border-viaduct-border px-3 py-2 text-sm text-viaduct-text-primary transition-colors hover:border-viaduct-accent hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
        aria-expanded={open}
      ...
      {open ? (
        <div
          ...
          className="fixed inset-x-3 top-20 z-50 rounded-lg border border-viaduct-border bg-viaduct-card shadow-2xl shadow-black/30 sm:absolute sm:right-0 sm:left-auto sm:top-12 sm:w-[28rem]"
        >
          <div className="border-b border-viaduct-border p-3">
            ...
            <input
              ...
              className="min-h-10 w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 text-sm text-viaduct-text-primary placeholder-stellar-text-secondary focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
            />
          </div>
          ...
          <div className="max-h-[60vh] overflow-y-auto p-2">
            {loading ? (
                ...
                {visibleItems.map((item, index) => (
                  <li key={item.id}>
                    <button
                      ...
                      className={`flex min-h-14 w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent ${
                        activeIndex === index
                          ? "bg-viaduct-accent text-white"
                          : "text-viaduct-text-primary hover:bg-viaduct-surface"
                      }`}
                    >
                      <span className="min-w-0">
                        ...
                        <span
                          className={`block truncate text-xs ${
                            activeIndex === index ? "text-white/80" : "text-viaduct-text-secondary"
                          }`}
                        >
                          {item.subtitle}
                        </span>
                        ...
                        <span
                          className={`rounded-full border px-2 py-0.5 text-[11px] ${
                            activeIndex === index
                              ...
                              : "border-viaduct-border text-viaduct-text-secondary"
                          }`}
                        >
              ...
              <div className="px-4 py-8 text-center">
                <p className="text-sm font-medium text-viaduct-text-primary">No entities found</p>
