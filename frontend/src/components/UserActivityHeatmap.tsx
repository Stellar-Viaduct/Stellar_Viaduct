    ...
    <section aria-label="User activity heatmap" className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-viaduct-text-secondary text-sm mt-0.5">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <input
            ...
            className="bg-viaduct-card border border-viaduct-border text-white text-sm rounded-lg px-3 py-1.5 placeholder:text-viaduct-text-secondary focus:outline-none focus:ring-1 focus:ring-viaduct-accent"
          />
          <select
            className="bg-viaduct-card border border-viaduct-border text-white text-sm rounded-lg px-3 py-1.5"
            value={rangeDays}
      ...
      {!isLoading && !error && (
        <div className="bg-viaduct-card border border-viaduct-border rounded-lg overflow-x-auto">
          {visibleUsers.length === 0 ? (
            <div className="p-10 text-center text-viaduct-text-secondary">
            ...
            <div className="p-6 relative" onMouseLeave={() => setTooltip(null)}>
              ...
              <div className="flex mb-2 ml-28">
                {Array.from({ length: 24 }, (_, h) => (
                  <div
                    key={h}
                    className="flex-1 text-center text-viaduct-text-secondary"
              ...
              {visibleUsers.map((user) => (
                <div key={user.actorId} className="flex items-center gap-2 mb-1.5">
                  <div
                    className="w-28 shrink-0 text-right pr-3 text-xs text-viaduct-text-secondary truncate"
                    title={user.actorId}
                  >
                  ...
                  <div className="text-xs text-viaduct-text-secondary w-10 text-right shrink-0">
                    {user.totalActions}
                  </div>
                </div>
              ))}
              ...
              <div className="flex mt-3 ml-28 gap-0.5">
                {DAY_LABELS.map((day) => (
                  <div
                    key={day}
                    className="text-viaduct-text-secondary text-center"
              ...
              <div className="flex items-center gap-2 mt-5">
                <span className="text-xs text-viaduct-text-secondary">Less</span>
                {["bg-stellar-border/30", "bg-blue-100/50", "bg-blue-200", "bg-blue-300", "bg-blue-400", "bg-blue-500"].map(
                  (cls) => (
                    <div key={cls} className={`w-3 h-3 rounded-sm ${cls}`} />
                  )
                )}
                <span className="text-xs text-viaduct-text-secondary">More</span>
              </div>
            </div>
          )}
        </div>
      ...
      {tooltip && (
        <div
          className="fixed z-50 bg-viaduct-card border border-viaduct-border rounded-lg p-3 text-xs shadow-xl pointer-events-none"
        ...
        >
          ...
          <p className="text-viaduct-accent font-medium">
            {tooltip.cell.count} action{tooltip.cell.count !== 1 ? "s" : ""}
          </p>
          {tooltip.cell.actions.length > 0 && (
            <p className="text-viaduct-text-secondary mt-1 truncate max-w-48">
