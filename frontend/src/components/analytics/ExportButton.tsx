...
function buildCsv(
  ...
  lines.push(`Stellar Viaduct Analytics Export`);
...
<head>
  <meta charset="UTF-8">
  <title>Stellar Viaduct Analytics - ${period}</title>
  <style>
...
<body>
  <h1>Stellar Viaduct Analytics Report</h1>
...
export default function ExportButton({
  ...
  const onCsv = () => {
    ...
    downloadFile(csv, `stellar-viaduct-analytics-${period.toLowerCase()}-${date}.csv`, "text/csv");
    setOpen(false);
  };
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={toggle}
        disabled={isDisabled}
        className="flex items-center gap-2 px-4 py-2 rounded-lg border border-viaduct-border bg-viaduct-card text-sm font-medium text-white hover:border-viaduct-accent/60 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
        </svg>
        Export
        <svg className="w-3.5 h-3.5 text-viaduct-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {open && (
        <div className="absolute right-0 mt-2 w-44 bg-viaduct-card border border-viaduct-border rounded-lg shadow-xl z-20 overflow-hidden">
          <button
            onClick={onCsv}
          ...
          <button
            onClick={onPdf}
            className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-white hover:bg-stellar-border/60 transition-colors border-t border-viaduct-border"
          >
