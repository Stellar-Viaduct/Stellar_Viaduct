...
export default function HealthDistribution({ data, isLoading }: HealthDistributionProps) {
  ...
  return (
    <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
      ...
      <p className="text-xs text-viaduct-text-secondary mb-4">{total} assets total</p>

      {data.length === 0 ? (
        <div className="h-56 flex items-center justify-center text-viaduct-text-secondary text-sm">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            {data.map((d) => (
              <div key={d.name} className="text-center">
                ...
                <p className="text-xs text-viaduct-text-secondary">{d.name}</p>
              </div>
            ))}
          </div>
        </>
