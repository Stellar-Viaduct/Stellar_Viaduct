...
const CustomTooltip = ({
  ...
  return (
    <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-3 text-xs shadow-xl">
      ...
      <p className="text-white font-semibold mt-1 pt-1 border-t border-viaduct-border flex justify-between">
        <span>Total</span>
...
export default function VolumeBreakdown({
  ...
  return (
    <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
      ...
      <p className="text-xs text-viaduct-text-secondary mb-4">
        Daily transfer volume by bridge over {period}
      </p>

      {data.length === 0 || bridgeNames.length === 0 ? (
        <div className="h-64 flex items-center justify-center text-viaduct-text-secondary text-sm">
