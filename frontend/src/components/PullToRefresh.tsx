...
export default function PullToRefresh({
  ...
  return (
    ...
    >
      <div
        className="mt-2 inline-flex items-center gap-3 rounded-full border border-viaduct-border bg-viaduct-card/95 px-4 py-2 text-xs text-white shadow-lg backdrop-blur"
        style={{
      ...
      >
        ...
        <span>{label}</span>
        <span className="text-viaduct-text-secondary">{Math.round(progress * 100)}%</span>
      </div>
    </div>
  );
}
