...
function PercentageMetric({
  ...
  return (
    <div className="flex justify-between items-center">
      <span className="text-sm text-viaduct-text-secondary">{label}</span>
      <span
        className="text-sm text-viaduct-text-primary font-medium"
...
function BridgeSummaryCardError({
  ...
  return (
    <div
      className={`bg-viaduct-card border border-viaduct-border rounded-lg p-6 ${className}`}
      role="alert"
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-viaduct-text-primary">
            Unable to load bridge summary
          </h3>
          {error && (
            <p className="text-xs text-viaduct-text-secondary mt-1">{error}</p>
          )}
        </div>
      </div>
    </div>
...
function CompactVariant({ summary }: { summary: BridgeSummary }) {
  return (
    <Link
      ...
      className="block bg-viaduct-card border border-viaduct-border rounded-lg hover:border-viaduct-accent transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent p-4"
      aria-label={`View bridge ${summary.name} with status ${summary.status}`}
    >
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-base font-semibold text-viaduct-text-primary truncate">
          {summary.name}
        </h3>
...
function StandardVariant({ summary }: { summary: BridgeSummary }) {
  return (
    <Link
      to={`/bridges?selected=${encodeURIComponent(summary.name)}`}
      className="block bg-viaduct-card border border-viaduct-border rounded-lg hover:border-viaduct-accent transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent p-6"
      aria-label={`View bridge ${summary.name}`}
    >
      {/* Header */}
      <div className="mb-4 flex items-center justify-between gap-2">
        <h3 className="text-lg font-semibold text-viaduct-text-primary truncate">
          {summary.name}
        </h3>
        <StatusBadge status={summary.status} />
      </div>
      ...
      <div className="space-y-3 mb-4">
        {/* Coverage Section */}
        <div>
          <div className="text-xs font-medium text-viaduct-text-secondary uppercase tracking-wide mb-2">
            Coverage
          </div>
      ...
      {/* Footer */}
      <div className="border-t border-viaduct-border pt-3">
      ...
      <div className="space-y-4 mb-4">
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            />
            <div className="flex justify-between items-center">
              <span className="text-sm text-viaduct-text-secondary">Mismatch</span>
              <span
