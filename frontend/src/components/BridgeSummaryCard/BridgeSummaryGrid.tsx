  ...
  if (isError) {
    return (
      <div className={`${gridClasses} ${className}`} role="alert">
        <div className="col-span-full bg-viaduct-card border border-viaduct-border rounded-lg p-6 text-center">
          <p className="text-viaduct-text-primary font-semibold">
            Unable to load bridges
          </p>
          {error && (
            <p className="text-viaduct-text-secondary text-sm mt-2">{error}</p>
          )}
        </div>
      </div>
    );
  ...
  if (summaries.length === 0) {
    return (
      <div className={`${gridClasses} ${className}`}>
        <div className="col-span-full bg-viaduct-card border border-viaduct-border rounded-lg p-8 text-center">
          <p className="text-viaduct-text-secondary">No bridges available</p>
        </div>
      </div>
    );
  }
