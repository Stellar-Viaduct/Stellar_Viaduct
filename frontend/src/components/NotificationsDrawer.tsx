...
const priorityTextColorMap: Record<NotificationPriority, string> = {
  ...
  low: "text-viaduct-text-secondary",
};
  ...
  return (
    <div className="fixed inset-0 z-50" aria-hidden={!open}>
      ...
      <div
        ...
        className="absolute right-0 top-0 flex h-full w-full flex-col border-l border-viaduct-border bg-viaduct-background/95 shadow-2xl shadow-black/40 transition-transform duration-300 sm:w-[24rem]"
      >
        <div className="border-b border-viaduct-border bg-viaduct-card/80 px-4 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-viaduct-text-primary">Notifications</h2>
              <p className="text-sm text-viaduct-text-secondary">{unreadCount} unread</p>
            </div>
            <button
              type="button"
              onClick={onClose}
              className="rounded-md px-2 py-1 text-sm text-viaduct-text-secondary transition hover:text-viaduct-text-primary focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
          ...
          <div className="mt-3 flex items-center gap-2">
            <button
              ...
              className="rounded-md border border-viaduct-border px-3 py-1.5 text-xs font-medium text-viaduct-text-primary transition hover:border-viaduct-accent disabled:cursor-not-allowed disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
            >
              Mark all as read
            </button>
            <button
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!hasAnyNotifications && (
            <p className="rounded-lg border border-viaduct-border bg-viaduct-card/60 p-4 text-sm text-viaduct-text-secondary">
  ...
  return (
    <li
      className={`rounded-xl border p-3 ${
        notification.read
          ? "border-viaduct-border bg-viaduct-card/50"
          : "border-viaduct-accent/40 bg-viaduct-accent/10"
      }`}
      role="listitem"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-viaduct-text-primary">{notification.title}</p>
          <p className="mt-1 text-sm text-viaduct-text-secondary">{notification.message}</p>
          <p className="mt-2 text-xs text-viaduct-text-secondary">
        ...
        <button
          ...
          className="rounded-md border border-viaduct-border px-2 py-1 text-xs font-medium text-viaduct-text-primary transition hover:border-viaduct-accent disabled:cursor-not-allowed disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
