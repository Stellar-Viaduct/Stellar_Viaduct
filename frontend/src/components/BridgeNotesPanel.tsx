...
function NoteCard({
  ...
  return (
    <article
      className="group rounded-lg border border-viaduct-border bg-viaduct-background/30 p-4 transition-colors hover:border-viaduct-accent/40"
      aria-label={`Note by ${note.author}`}
    >
      {/* header */}
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-viaduct-accent/20 text-xs font-semibold text-viaduct-accent uppercase">
            {note.author[0]}
          </span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <time
            ...
            className="text-xs text-viaduct-text-secondary"
            title={note.updatedAt !== note.createdAt ? `Edited ${formatTimestamp(note.updatedAt)}` : undefined}
          >
            {formatTimestamp(note.createdAt)}
            {note.updatedAt !== note.createdAt && (
              <span className="ml-1 text-viaduct-text-secondary/60">(edited)</span>
            )}
          </time>
          ...
          {isOwner && !editing && (
            <>
              <button
                ...
                className="ml-1 rounded p-1 text-viaduct-text-secondary opacity-0 transition-opacity hover:text-white group-hover:opacity-100 focus:opacity-100"
              >
              ...
              <button
                ...
                className="rounded p-1 text-viaduct-text-secondary opacity-0 transition-opacity hover:text-red-400 group-hover:opacity-100 focus:opacity-100"
              >
                <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
      ...
      {editing ? (
        <div className="space-y-2">
          <textarea
            ...
            rows={4}
            className="w-full rounded border border-viaduct-border bg-viaduct-card px-3 py-2 text-sm text-white placeholder-stellar-text-secondary focus:outline-none focus:ring-2 focus:ring-viaduct-accent resize-none"
            placeholder="Write your note… (**bold**, _italic_)"
            aria-label="Edit note content"
          />
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs text-viaduct-text-secondary">
              Tip: <kbd className="rounded border border-viaduct-border px-1">Ctrl+Enter</kbd> to save
            </p>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => { setEditing(false); setDraft(note.content); }}
                className="rounded border border-viaduct-border px-3 py-1 text-xs text-viaduct-text-secondary hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                onClick={handleSave}
                disabled={!draft.trim()}
                className="rounded bg-viaduct-accent px-3 py-1 text-xs font-medium text-white hover:bg-viaduct-accent/80 disabled:opacity-40 transition-colors"
              >
                Save
              </button>
            </div>
        ...
        <p className="text-sm text-viaduct-text-primary leading-relaxed whitespace-pre-wrap">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2">
      ...
      <textarea
        ...
        className="w-full rounded-lg border border-viaduct-border bg-viaduct-card px-3 py-2 text-sm text-white placeholder-stellar-text-secondary focus:outline-none focus:ring-2 focus:ring-viaduct-accent resize-none"
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          type="submit"
          disabled={!value.trim()}
          className="inline-flex items-center gap-1.5 rounded-lg bg-viaduct-accent px-4 py-1.5 text-sm font-medium text-white hover:bg-viaduct-accent/80 disabled:opacity-40 transition-colors focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
        >
          <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
...
export default function BridgeNotesPanel({ bridgeName }: BridgeNotesPanelProps) {
  ...
  return (
    <section
      ...
      className="rounded-xl border border-viaduct-border bg-viaduct-card p-6 space-y-5"
    >
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2
          ...
          <p className="mt-0.5 text-sm text-viaduct-text-secondary">
            Internal coordination notes for <span className="font-medium text-white">{bridgeName}</span>
          </p>
        </div>
        {notes.length > 0 && (
          <span className="rounded-full bg-viaduct-accent/15 px-2.5 py-0.5 text-xs font-semibold text-viaduct-accent">
            {notes.length}
          </span>
        )}
      </div>
      ...
      {notes.length > 0 ? (
        ...
        <div className="rounded-lg border border-dashed border-viaduct-border py-8 text-center">
          <svg
            className="mx-auto mb-2 text-viaduct-text-secondary"
            width="32"
            height="32"
            viewBox="0 0 24 24"
            fill="none"
          ...
          <p className="text-sm text-viaduct-text-secondary">No notes yet</p>
          <p className="mt-1 text-xs text-viaduct-text-secondary/60">
