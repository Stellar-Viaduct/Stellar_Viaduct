      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded-md border border-viaduct-border px-3 py-2 text-sm text-white hover:bg-stellar-border"
          >
            Export
          </button>
          <button
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            <label className="space-y-2 text-sm text-viaduct-text-secondary">
              <span className="block text-white">Type</span>
              <select
                value={kind}
                onChange={(event) => setKind(event.target.value as ChartAnnotationInput["kind"])}
                className="w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
              >
          ...
          <label className="space-y-2 text-sm text-viaduct-text-secondary">
            ...
            <input
              ...
              className="w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-white placeholder:text-viaduct-text-secondary focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
            />
          </label>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <label className="flex items-center gap-2 text-sm text-viaduct-text-secondary">
              ...
              <input
                ...
                className="h-9 w-12 rounded border border-viaduct-border bg-transparent"
              />
            </label>
            ...
            <button
              ...
              onClick={submitForm}
              disabled={!canSubmit}
              className="rounded-md bg-viaduct-accent px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            {editingId ? (
              <button
                ...
                className="rounded-md border border-viaduct-border px-4 py-2 text-sm text-viaduct-text-secondary hover:text-white"
              >
                Cancel edit
              </button>
            ) : null}
        ...
        <div className="rounded-lg border border-viaduct-border/80 bg-viaduct-background/30 p-4">
          <div className="mb-3 flex items-center justify-between">
            <h4 className="text-sm font-semibold text-white">Saved annotations</h4>
            <span className="text-xs text-viaduct-text-secondary">{annotations.length} total</span>
          </div>
          ...
          {annotations.length === 0 ? (
            ...
            <ul className="space-y-3">
              {annotations.map((annotation) => (
                <li key={annotation.id} className="rounded-md border border-viaduct-border p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="rounded-full border border-viaduct-border px-2 py-0.5 text-[11px] uppercase tracking-wider text-viaduct-text-secondary">
                        {annotation.kind}
                      </span>
                    </div>
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-xs text-viaduct-accent hover:underline"
                      >
                        Edit
                      </button>
                      <button
                  ...
                  {annotation.notes ? (
                    <p className="mt-2 text-sm text-viaduct-text-secondary">{annotation.notes}</p>
                  ) : null}
                  <p className="mt-2 text-xs text-viaduct-text-secondary">{annotation.timestamp}</p>
                </li>
              ))}
            </ul>
          )}
