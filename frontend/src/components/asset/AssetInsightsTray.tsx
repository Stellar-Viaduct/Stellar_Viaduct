...
function TrendSummaryCard({ health, loading }: { health: HealthScore | null | undefined; loading: boolean }) {
  if (loading) {
    return (
      <div className="rounded-lg border border-viaduct-border bg-viaduct-card p-4 animate-pulse">
  ...
  if (!health) {
    return (
      <div className="rounded-lg border border-viaduct-border bg-viaduct-card p-4">
        <p className="text-xs uppercase text-viaduct-text-secondary">Trend Summary</p>
        <p className="mt-2 text-sm text-viaduct-text-secondary">No health data available.</p>
      </div>
    );
  }
  ...
  return (
    <section aria-label="Trend summary" className="rounded-lg border border-viaduct-border bg-viaduct-card p-4 space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-wide text-viaduct-text-secondary">
          Trend Summary
        </p>
        {trend ? (
      ...
      <div className="flex items-end gap-3">
        ...
        <p className="mb-1 text-xs text-viaduct-text-secondary">/ 100 health score</p>
      </div>
        ...
        {Object.entries(factors).map(([key, value]) => {
          ...
          return (
            <div key={key} className="flex flex-col gap-1">
              <p className="text-xs text-viaduct-text-secondary truncate">{label}</p>
...
function RecentAlertsCard({ alerts, loading }: { alerts: AssetAlert[]; loading: boolean }) {
  if (loading) {
    return (
      <div className="rounded-lg border border-viaduct-border bg-viaduct-card p-4 animate-pulse space-y-3">
        <p className="h-3 w-24 rounded bg-stellar-border/60" />
        {[0, 1, 2].map((i) => (
  ...
  return (
    <section aria-label="Recent alerts" className="rounded-lg border border-viaduct-border bg-viaduct-card p-4 space-y-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-viaduct-text-secondary">
        Recent Alerts
      </p>

      {alerts.length === 0 ? (
        <p className="text-sm text-viaduct-text-secondary">No recent alerts.</p>
      ) : (
...
function VolumeCard({ volume, loading }: {
  ...
  return (
    <section aria-label="Volume overview" className="rounded-lg border border-viaduct-border bg-viaduct-card p-4 space-y-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-viaduct-text-secondary">
        Volume
      </p>
      <div className="grid grid-cols-3 gap-2">
        {periods.map(({ label, value }) => (
          <div key={label} className="flex flex-col gap-1">
            <p className="text-xs text-viaduct-text-secondary">{label}</p>
...
function QuickActionsCard({
  ...
  return (
    <section aria-label="Quick actions" className="rounded-lg border border-viaduct-border bg-viaduct-card p-4 space-y-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-viaduct-text-secondary">
        Quick Actions
      </p>
      <div className="flex flex-col gap-2">
        <Link
          to={`/assets/${symbol}`}
          onClick={onClose}
          className="flex items-center justify-between rounded-md border border-viaduct-border px-3 py-2 text-sm font-medium text-viaduct-text-primary transition-colors hover:border-viaduct-accent hover:text-white focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
        >
          View full details
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <aside
        ...
        className="absolute right-0 top-0 flex h-full w-full flex-col border-l border-viaduct-border bg-viaduct-background shadow-2xl shadow-black/40 sm:w-[26rem] lg:w-[30rem]"
      >
        {/* Header */}
        <div className="border-b border-viaduct-border bg-viaduct-card/80 px-4 py-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-wide text-viaduct-text-secondary">
                Asset Insights
              </p>
              <h2
              ...
              {displayName !== symbol ? (
                <p className="mt-0.5 text-sm text-viaduct-text-secondary">{symbol}</p>
              ) : null}
            </div>
            <button
              type="button"
              onClick={onClose}
              className="shrink-0 rounded-md px-2 py-1 text-sm text-viaduct-text-secondary transition hover:text-white focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
