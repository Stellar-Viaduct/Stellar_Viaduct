...
export default function MetricsDrilldown({
  ...
  return (
    ...
    >
      <div
        className={`bg-viaduct-card border border-viaduct-border rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col transition-all duration-300 ${
          isClosing ? "opacity-0 scale-95" : "opacity-100 scale-100"
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-viaduct-border bg-viaduct-background/30">
          <h2 className="text-xl font-semibold text-white">{title}</h2>
          <button
            type="button"
            onClick={onClose}
            className="text-viaduct-text-secondary hover:text-white transition-colors"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div
                  key={i}
                  className="bg-viaduct-card border border-viaduct-border rounded-lg p-5 animate-pulse"
                >
          ...
          <div className="mt-8">
            ...
            <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
              {loading ? (
                ...
                <div className="text-viaduct-text-secondary text-center py-8">
          ...
          <div className="mt-8">
            ...
            <div className="bg-viaduct-card border border-viaduct-border rounded-lg overflow-hidden">
              {loading ? (
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-viaduct-border bg-viaduct-background/30 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-viaduct-accent text-white rounded-lg hover:bg-viaduct-accent/80 transition-colors"
          >
            Close
          </button>
        </div>
