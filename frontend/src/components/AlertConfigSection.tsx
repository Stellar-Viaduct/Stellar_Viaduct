...
export default function AlertConfigSection({ alerts, isLoading }: AlertConfigSectionProps) {
  ...
  if (isLoading) {
    return (
      <div className="bg-viaduct-card border border-viaduct-border rounded-xl p-6">
  ...
  return (
    <div className="bg-viaduct-card border border-viaduct-border rounded-xl p-6">
      ...
      {alerts && alerts.length > 0 ? (
        <div className="space-y-3">
          {alerts.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className="flex flex-col gap-3 rounded-lg border border-viaduct-border bg-viaduct-background p-3"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {status ? (
                      <p className="mt-1 text-xs text-viaduct-accent">
        ...
        <p className="text-viaduct-text-secondary text-sm">No active alerts.</p>
      )}
    </div>
  );
}
