...
export default function TimeRangeSelector({
  ...
  return (
    <div className={`rounded-md border border-viaduct-border p-3 ${className}`}>
      ...
      {showApplyGlobally ? (
        <label className="mt-3 inline-flex items-center gap-2 text-xs text-viaduct-text-secondary">
          <input
            type="checkbox"
            checked={applyGlobally}
            onChange={(event) => setApplyGlobally(event.target.checked)}
            className="h-4 w-4 rounded border-viaduct-border bg-viaduct-background"
          />
          Apply to all charts
        </label>
      ) : null}
      ...
      {!applyGlobally ? (
        ...
        <p className="mt-2 text-xs text-viaduct-text-secondary">Global scope enabled</p>
      ) : null}
