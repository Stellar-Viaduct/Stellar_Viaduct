...
function getRecentRanges(): TimeRangeSelection[] {
  ...
  try {
    const stored = window.localStorage.getItem("viaduct.recentRanges.v1");
    if (!stored) {
      return [];
    }
...
function saveRecentRange(selection: TimeRangeSelection): void {
  ...
  try {
    ...
    window.localStorage.setItem("viaduct.recentRanges.v1", JSON.stringify(updated));
  } catch {
...
export default function DateRangePicker({
  ...
  return (
    <div
      ref={containerRef}
      className="mt-3 space-y-3 rounded-md border border-viaduct-border p-3"
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <label className="text-xs text-viaduct-text-secondary">
          ...
          <input
            ...
            className="mt-1 w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-white transition-all focus:outline-none focus:ring-2 focus:ring-viaduct-accent/50"
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium text-viaduct-text-secondary">Presets</p>
        <div
          ...
          {TIME_RANGE_PRESETS.map((preset) => {
            ...
            return (
              <button
                ...
                className={`rounded px-2.5 py-1 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-viaduct-accent/50 ${
                  isActive
      ...
      {recentRanges.length > 0 ? (
        <div className="space-y-2">
          <p className="text-xs font-medium text-viaduct-text-secondary">Recent</p>
            ...
            {recentRanges.map((range, index) => {
              ...
              return (
                <button
                  ...
                  className="rounded border border-viaduct-border px-2.5 py-1 text-xs text-viaduct-text-secondary transition-colors hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-viaduct-accent/50"
                  title={label}
                >
                  {label}
                </button>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          disabled={!canApply}
          className="rounded bg-viaduct-accent px-3 py-1.5 text-xs font-medium text-white transition-all disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-viaduct-accent/50"
        >
          Apply custom range
        </button>
        ...
        <button
          type="button"
          onClick={handleClear}
          className="rounded border border-viaduct-border px-3 py-1.5 text-xs text-viaduct-text-secondary transition-colors hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-viaduct-accent/50"
        >
          Clear
        </button>
      </div>
