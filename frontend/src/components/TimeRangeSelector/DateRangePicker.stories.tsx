...
const meta = {
  title: "Stellar Viaduct/Components/DateRangePicker",
  ...
  render: () => {
    ...
    return (
      <div className="space-y-4">
        ...
        />
        {applied && (
          <div className="rounded-md border border-viaduct-border p-3">
            <p className="text-xs font-medium text-viaduct-text-secondary">Applied Range:</p>
            <pre className="mt-2 text-xs text-viaduct-text-primary">
    ...
    return (
      <div className="space-y-4">
        <div className="rounded-md border border-viaduct-border p-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="text-xs text-viaduct-text-secondary">
              ...
              <input
                ...
                className="mt-1 w-full rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-white"
              />
            </label>
            <label className="text-xs text-viaduct-text-secondary">
              End
          ...
          <button
            disabled
            className="mt-3 rounded bg-viaduct-accent px-3 py-1.5 text-xs font-medium text-white disabled:opacity-50"
          >
            Apply custom range
          </button>
        </div>
...
export const WithRecentRanges: Story = {
  render: () => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("stellarviaduct.recentRanges.v1", JSON.stringify(recentRanges));
    }
...
export const Interactive: Story = {
  render: () => {
    ...
    if (typeof window !== "undefined" && showRecent) {
      const existing = localStorage.getItem("stellarviaduct.recentRanges.v1");
      if (!existing) {
        const recentRanges: TimeRangeSelection[] = [
          {
            start: "2026-04-15T00:00:00.000Z",
    ...
    return (
      <div className="space-y-4">
          ...
          <label className="text-xs text-viaduct-text-secondary">
            <input
              ...
              onChange={(e) => {
                ...
                if (!e.target.checked) {
                  localStorage.removeItem("stellarviaduct.recentRanges.v1");
                }
              }}
              className="mr-2"
            />
        ...
        <div className="rounded-md border border-viaduct-border p-3">
          <p className="text-xs font-medium text-viaduct-text-secondary">Features:</p>
          <ul className="mt-2 space-y-1 text-xs text-viaduct-text-secondary">
