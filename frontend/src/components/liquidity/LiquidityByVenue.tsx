...
const CustomTooltip = ({ active, payload }: CustomTooltipProps) => {
  ...
  return (
    <div className="bg-viaduct-background border border-viaduct-border rounded-lg p-3 text-xs shadow-lg">
      ...
      <p className="text-viaduct-text-secondary">{inner.share.toFixed(1)}% share</p>
    </div>
  );
};
...
const LiquidityByVenue = React.memo(function LiquidityByVenue({
  ...
  if (chartData.length === 0) {
    return (
      <div className="h-48 flex items-center justify-center text-viaduct-text-secondary text-sm">
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <PieChart>
        ...
        <Legend
          formatter={(value) => (
            <span className="text-xs text-viaduct-text-secondary">{value}</span>
          )}
        />
      </PieChart>
    </ResponsiveContainer>
