...
export function TableFooterSummary({
  ...
  return (
    <tfoot
      ...
      className={[
        "border-t-2 border-viaduct-accent/30 bg-viaduct-background/60",
      ...
      {summaryNote && (
        <tr>
          <td
            ...
            className="pb-3 pt-0 pr-4 text-xs text-viaduct-text-secondary"
          >
            {summaryNote}
          </td>
        </tr>
...
export function TableSummaryCard({ items, note, className = "" }: TableSummaryCardProps) {
  return (
    <div
      ...
      className={[
        "rounded-lg border border-viaduct-border bg-viaduct-card px-4 py-3",
        className,
      ].join(" ")}
    >
      <dl className="flex flex-wrap gap-x-6 gap-y-2">
        {items.map((item) => (
          <div key={item.label} className="flex items-baseline gap-2">
            <dt className="text-xs text-viaduct-text-secondary whitespace-nowrap">
              {item.label}
            </dt>
      ...
      {note && (
        <p className="mt-2 text-xs text-viaduct-text-secondary">{note}</p>
      )}
    </div>
  );
}
