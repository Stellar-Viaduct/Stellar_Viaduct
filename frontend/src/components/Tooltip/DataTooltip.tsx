  ...
  return (
    <div className="space-y-1 min-w-[120px] max-w-xs">
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-viaduct-text-primary">{label}</span>
        {badge && <span>{badge}</span>}
      </div>

      {value !== undefined && (
        <div className="text-sm font-medium text-viaduct-text-primary tabular-nums">
          {value}
        </div>
      )}

      {description && (
        <div className="text-xs text-viaduct-text-secondary leading-relaxed">{description}</div>
      )}
