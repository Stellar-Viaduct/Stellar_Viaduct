...
function ScheduleRow({ schedule: s, onToggle, onDelete, onRunNow }: RowProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-3 rounded-lg border border-viaduct-border bg-viaduct-background p-4">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          ...
          className="rounded border border-viaduct-border px-3 py-1.5 text-xs text-viaduct-text-secondary hover:text-white transition-colors"
          title="Run now"
        >
          Run now
        </button>
        ...
        <button
          ...
          className={`rounded border px-3 py-1.5 text-xs transition-colors ${
            s.isActive
              ? "border-viaduct-border text-viaduct-text-secondary hover:text-yellow-400"
        ...
        {confirming ? (
          ...
          <button
            ...
            className="rounded border border-viaduct-border px-3 py-1.5 text-xs text-viaduct-text-secondary hover:text-red-400 transition-colors"
...
export default function ScheduleList() {
  ...
  if (schedules.length === 0) {
    return (
      <div className="rounded-lg border border-viaduct-border p-8 text-center">
        <svg
