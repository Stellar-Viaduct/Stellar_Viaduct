...
const CustomTooltip = ({
  ...
  return (
    <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-3 text-xs shadow-xl min-w-[160px]">
      <p className="text-viaduct-text-secondary mb-1">
        {label ? formatXAxis(label, period) : ""}
      </p>
      <p className="text-white font-semibold text-base" style={{ color: scoreColor(score) }}>
        {score}
        <span className="text-viaduct-text-secondary font-normal ml-1">/ 100</span>
      </p>
      {point?.annotation && (
...
export default function HealthTimelineChart({ points, period, bridgeName, isMockData }: Props) {
  if (points.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 text-viaduct-text-secondary text-sm">
  ...
  return (
    <div className="relative">
      {isMockData && (
        <div className="absolute top-0 right-0 z-10 text-xs text-stellar-text-muted bg-viaduct-background/80 rounded px-2 py-1">
          Demo data
        </div>
      )}
