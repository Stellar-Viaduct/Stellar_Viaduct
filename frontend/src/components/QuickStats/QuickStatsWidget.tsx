...
export default function QuickStatsWidget({ assets, bridges, isLoading }: QuickStatsProps) {
  ...
  const headerActions = stats.length > 4 ? (
    <button
      ...
      className="text-sm text-viaduct-accent hover:underline focus:outline-none focus:ring-2 focus:ring-viaduct-accent rounded-md px-2 py-1"
  ...
  if (isLoading) {
    return (
      <CollapsibleWidget
        ...
        headerClassName="text-xl font-semibold text-viaduct-text-primary"
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="bg-viaduct-card border border-viaduct-border rounded-lg p-4 animate-pulse"
