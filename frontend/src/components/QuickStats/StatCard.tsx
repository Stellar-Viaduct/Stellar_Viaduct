...
const STATUS_COLORS: Record<string, string> = {
  ...
  neutral: "border-viaduct-border hover:border-viaduct-accent",
};
...
const CHANGE_COLORS: Record<string, string> = {
  up: "text-green-400",
  down: "text-red-400",
  neutral: "text-viaduct-text-secondary",
};
...
export default function StatCard({ stat }: StatCardProps) {
  ...
  const content = (
    <div
      className={`bg-viaduct-card border ${borderClass} rounded-lg p-4 transition-colors`}
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-viaduct-text-secondary font-medium uppercase tracking-wide">
            {stat.label}
          </span>
        </div>
      ...
      <div className="text-2xl font-bold text-viaduct-text-primary">{stat.value}</div>
      {stat.change && (
  ...
  if (stat.href) {
    return (
      <Link
        to={stat.href}
        className="block focus:outline-none focus:ring-2 focus:ring-viaduct-accent rounded-lg"
