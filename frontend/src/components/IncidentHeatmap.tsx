...
export default function IncidentHeatmap() {
  ...
  return (
    <section aria-label="Incident heatmap" className="overflow-auto p-4">
      ...
      {isLoading && <p className="text-viaduct-text-secondary">Loading…</p>}
      {!isLoading && !error && (
        <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${assetsSet.length + 1}, minmax(30px, 1fr))` }}>
          {/* Header row */}
          <div></div>
          {assetsSet.map((asset) => (
            <div key={asset} className="text-xs text-center text-viaduct-text-secondary" title={asset}>
              {asset}
            </div>
          ))}
          {/* Data rows */}
          {dates.map((date) => (
            <React.Fragment key={date}>
              <div className="text-xs text-viaduct-text-secondary" title={date}>
                {date}
              </div>
              {assetsSet.map((asset) => {
                ...
                const color = count > 0 ? getColor(avgScore) : "bg-viaduct-card";
                return (
                  <div
                    key={asset}
