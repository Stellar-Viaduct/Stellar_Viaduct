...
export default function Watchlist() {
  ...
  return (
    <div className="space-y-4 rounded-lg border border-viaduct-border bg-viaduct-card p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            className="rounded-md border border-viaduct-border bg-viaduct-background px-3 py-2 text-sm text-white"
          ...
          <button
            ...
            className="rounded border border-viaduct-border px-3 py-2 text-xs text-viaduct-text-secondary hover:text-white"
          >
            New list
          </button>
          ...
          <button
            ...
            className="rounded border border-viaduct-border px-3 py-2 text-xs text-red-300 hover:text-red-200"
          >
            Delete
          </button>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          onClick={() => {
            ...
            anchor.href = url;
            anchor.download = "stellarviaduct-watchlists.json";
            anchor.click();
            URL.revokeObjectURL(url);
          }}
          className="rounded border border-viaduct-border px-3 py-1.5 text-xs text-viaduct-text-secondary hover:text-white"
        >
          Export
        </button>
      ...
      {!activeWatchlist || activeWatchlist.assets.length === 0 ? (
        <p className="text-sm text-viaduct-text-secondary">No assets in this watchlist yet.</p>
      ) : (
        <ul className="space-y-2">
          {activeWatchlist.assets.map((symbol) => (
            <li
              key={symbol}
              className="flex flex-wrap items-center justify-between gap-2 rounded border border-viaduct-border px-3 py-2"
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="rounded border border-viaduct-border px-2 py-1 text-xs text-red-300 hover:text-red-200"
                >
                  Remove
                </button>
              </div>
