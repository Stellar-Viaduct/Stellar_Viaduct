...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-3 text-xs shadow-xl">
      <p className="text-viaduct-text-secondary mb-1">{label}</p>
      {payload
        .filter((p) => p.value > 0)
    ...
    <section aria-label="Anomaly trend charts" className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex rounded-lg border border-viaduct-border overflow-hidden">
          ...
          <button
            ...
            disabled={!trendData}
            className="px-3 py-1.5 bg-viaduct-card border border-viaduct-border text-viaduct-text-secondary text-sm rounded-lg hover:text-white transition-colors disabled:opacity-50"
          >
            Export CSV
          </button>
        </div>
      ...
      {trendData && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {(["critical", "high", "medium", "low"] as AnomalySeverity[]).map((sev) => (
            ...
            >
              <p className="text-viaduct-text-secondary text-xs uppercase tracking-wide mb-1">
                {SEVERITY_LABELS[sev]}
              </p>
      ...
      <div className="bg-viaduct-card border border-viaduct-border rounded-lg p-6">
        <h3 className="text-sm font-medium text-viaduct-text-secondary uppercase tracking-wide mb-4">
              ...
              return (
                <div key={asset} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-stellar-border rounded-full h-2">
                    <div
                      className="bg-viaduct-accent h-2 rounded-full transition-all"
                      style={{ width: `${(count / max) * 100}%` }}
                    />
                  </div>
                  <span className="text-viaduct-text-secondary text-sm w-8 text-right">{count}</span>
                </div>
              );
            })}
          </div>
