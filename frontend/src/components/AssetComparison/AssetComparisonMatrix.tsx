...
function trendColor(trend: string | undefined): string {
  if (!trend) return "text-viaduct-text-secondary";
...
export default function AssetComparisonMatrix({ assets, filter }: Props) {
  ...
  if (assets.length === 0) {
    return (
      <div className="text-center py-12 text-viaduct-text-secondary text-sm">
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-1">
          <span className="text-xs text-viaduct-text-secondary mr-1">Columns:</span>
            ...
            <button
              ...
              className={`rounded px-2 py-0.5 text-xs transition-colors focus:outline-none ${
                visibleCols.has(col.id)
                  ? "bg-viaduct-accent/20 text-viaduct-accent border border-viaduct-accent/40"
                  : "border border-viaduct-border text-stellar-text-muted"
              }`}
            >
              {col.label}
            </button>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded border border-viaduct-border px-3 py-1.5 text-xs text-viaduct-text-secondary hover:text-white transition-colors"
        >
      ...
      {/* Table */}
      <div className="overflow-x-auto rounded-lg border border-viaduct-border">
        <table className="w-full text-sm" aria-label="Asset comparison matrix">
          <thead>
            <tr className="border-b border-viaduct-border bg-viaduct-background">
              {visibleColumns.map((col) => (
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-viaduct-text-secondary hover:text-white transition-colors focus:outline-none"
                    title={col.hint}
                    aria-sort={
                      sortCol === col.id
                        ? sortDir === "asc"
                  ...
                  >
                    {col.label}
                    {sortCol === col.id && (
                      <span className="text-viaduct-accent">{sortDir === "asc" ? "↑" : "↓"}</span>
                    )}
                  </button>
                </th>
              ))}
          ...
          <tbody>
            {sorted.map((asset) => (
              <tr
                key={asset.symbol}
                className="border-b border-viaduct-border/50 hover:bg-viaduct-surface/40 transition-colors"
              >
                {visibleColumns.map((col) => {
                  if (col.id === "symbol") {
                    return (
                      <td key={col.id} className="px-4 py-3">
                        ...
                        <br />
                        <span className="text-xs text-viaduct-text-secondary">{asset.name}</span>
                      </td>
                    );
                  }
                  if (col.id === "trend") {
            ...
            {sorted.length === 0 && (
              <tr>
                <td
                  ...
                  className="px-4 py-8 text-center text-viaduct-text-secondary"
                >
