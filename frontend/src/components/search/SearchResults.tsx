    ...
    <>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <mark
            key={i}
            className="bg-viaduct-accent/30 text-white rounded-sm px-0.5"
          >
            {part}
          </mark>
        ) : (
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors rounded-lg ${
        isActive
          ? "bg-viaduct-accent/20 text-white"
          : "text-viaduct-text-secondary hover:bg-stellar-border/50 hover:text-white"
      }`}
    ...
    >
      ...
      <span
        className={`flex-none w-8 h-8 flex items-center justify-center rounded-md text-base font-mono ${
          isActive ? "bg-viaduct-accent text-white" : "bg-stellar-border text-viaduct-text-secondary"
        }`}
      >
        {icon}
      </span>
      ...
      <span className="min-w-0 flex-1">
        ...
        {result.subtitle && (
          <span className="block text-xs text-viaduct-text-secondary truncate mt-0.5">
      ...
      {/* Arrow hint */}
      {isActive && (
        <span className="flex-none text-viaduct-text-secondary text-xs">↵</span>
      )}
    </button>
  );
}
      ...
      {CATEGORY_ORDER.map((cat) => {
        ...
        return (
          <div key={cat} className="mb-2">
            <p className="px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-viaduct-text-secondary">
              {label}
            </p>
            {items.map((result) => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-4 py-1.5">
        <p className="text-xs font-semibold uppercase tracking-wider text-viaduct-text-secondary">
          Recent
        </p>
        <button
          type="button"
