...
const ACTIVITY_FILTERS_STORAGE_KEY = "stellar-viaduct:activity-filters";
  ...
  return (
    <section className={`space-y-4 ${className}`} aria-label="Recent Activity Timeline">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 bg-viaduct-card border border-viaduct-border rounded hover:bg-viaduct-card-hover transition-colors"
              aria-label={`Switch to ${displayMode === "compact" ? "expanded" : "compact"} mode`}
              title={`Switch to ${displayMode === "compact" ? "expanded" : "compact"} mode`}
            >
              {displayMode === "compact" ? (
                <svg className="w-4 h-4 text-viaduct-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            >
              <svg
                className={`w-4 h-4 text-viaduct-text-secondary transition-transform ${sortOrder === "oldest" ? "rotate-180" : ""}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            {totalEvents > 0 && (
              <button
                onClick={clearEvents}
                className="px-3 py-2 bg-viaduct-card border border-viaduct-border rounded text-xs text-red-400 hover:bg-viaduct-card-hover transition-colors"
      ...
      {isLoading && (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <article
              key={i}
              className="border border-viaduct-border rounded-lg p-4 animate-pulse bg-viaduct-card"
              aria-busy="true"
            >
      ...
      {!isLoading && !error && events.length === 0 && (
        <div className="border border-viaduct-border rounded-lg p-8 text-center">
          <svg
