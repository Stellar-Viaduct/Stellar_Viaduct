      ...
      return (
        <div className="mt-3 space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="text-viaduct-text-secondary font-medium">{event.bridgeName}</span>
          </div>
          <div className="flex justify-between">
          ...
          {event.totalValueLocked !== undefined && (
            <div className="flex justify-between">
              ...
              <span className="text-viaduct-text-secondary font-medium">
    ...
    case "health":
      return (
        <div className="mt-3 space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-stellar-text-muted">Asset:</span>
            <span className="text-viaduct-text-secondary font-medium">{event.assetSymbol}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-stellar-text-muted">Previous Score:</span>
            <span className="text-viaduct-text-secondary">{event.previousScore.toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
    ...
    case "transaction":
      return (
        <div className="mt-3 space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-stellar-text-muted">Bridge:</span>
            <span className="text-viaduct-text-secondary font-medium">{event.bridge}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-stellar-text-muted">Asset:</span>
            <span className="text-viaduct-text-secondary">{event.asset}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-viaduct-text-secondary text-xs">
              {event.sourceChain} → {event.destinationChain}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-stellar-text-muted">Tx Hash:</span>
            <code className="text-viaduct-text-secondary text-xs font-mono truncate max-w-[200px]">
              {event.txHash}
            </code>
          </div>
        </div>
        ...
        <div className="mt-3 space-y-2 text-sm">
          ...
          {event.alertType && (
            <div className="flex justify-between">
              ...
              <span className="text-viaduct-text-secondary capitalize">{event.alertType}</span>
            </div>
          )}
        </div>
      );
...
export default function TimelineEventCard({ event, mode = "compact", onRemove }: TimelineEventCardProps) {
  ...
  const cardContent = (
    <article
      className="border border-viaduct-border rounded-lg p-4 bg-viaduct-card hover:bg-viaduct-card-hover transition-colors cursor-pointer"
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-grow min-w-0">
          ...
          <p className="text-sm text-viaduct-text-secondary mt-1 line-clamp-2">
            {event.description}
          </p>
          ...
          {isExpanded && (
            <div className="mt-3 flex gap-2">
              {link && (
                <Link
                  to={link}
                  className="text-xs text-viaduct-accent hover:underline inline-flex items-center gap-1"
