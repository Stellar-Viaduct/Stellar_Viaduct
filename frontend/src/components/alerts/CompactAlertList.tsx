  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 whitespace-nowrap text-xs font-semibold uppercase tracking-wider transition-colors ${
        active
          ? "text-viaduct-accent"
          : "text-stellar-text-muted hover:text-viaduct-text-secondary"
      }`}
    >
      {label}
      {active ? (
...
function AlertRow({
  ...
  return (
    <div
      role="row"
      className={`flex items-center gap-2 px-3 ${py} cursor-pointer transition-colors hover:bg-viaduct-card-hover border-b border-viaduct-border/50 last:border-b-0 ${
        selected
          ? "bg-viaduct-accent/10"
          : isUnread
          ? "bg-viaduct-card"
          : "bg-viaduct-card/40"
      }`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 min-w-0">
          {isUnread && (
            <span
              className="w-1.5 h-1.5 rounded-full bg-viaduct-accent flex-shrink-0"
              aria-hidden="true"
            />
          )}
          <span
    ...
    <section className={`space-y-3 ${className}`} aria-label="Compact Alert List">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div
          ...
          className="flex items-center gap-0.5 rounded-md border border-viaduct-border p-0.5 bg-viaduct-background/40"
        >
          <button
            type="button"
            ...
            className={`flex items-center justify-center w-8 h-7 rounded transition-colors ${
              density === "compact"
                ? "bg-viaduct-card text-white shadow-sm"
                : "text-stellar-text-muted hover:text-viaduct-text-secondary"
            }`}
          >
      ...
      <div className="flex flex-wrap gap-2">
        <input
          ...
          className="bg-viaduct-card border border-viaduct-border rounded px-3 py-1.5 text-sm text-white placeholder-stellar-text-muted focus:outline-none focus:border-viaduct-accent w-full sm:w-56"
        ...
        <select
          ...
          className="bg-viaduct-card border border-viaduct-border rounded px-3 py-1.5 text-sm text-white focus:outline-none focus:border-viaduct-accent"
      ...
      {selectedIds.size > 0 && (
        <div
          ...
          className="flex items-center gap-3 px-3 py-2 bg-viaduct-accent/10 border border-viaduct-accent/30 rounded-lg"
        >
          <span className="text-sm text-viaduct-text-primary font-medium">
          ...
          <div className="flex gap-2 ml-auto">
            <button
              ...
              className="px-3 py-1.5 text-xs rounded-md border border-viaduct-border text-viaduct-text-secondary hover:text-white hover:border-viaduct-accent transition-colors"
            >
              Mark read
            </button>
            <button
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs rounded-md text-stellar-text-muted hover:text-viaduct-text-secondary transition-colors"
            >
              Clear
            </button>
          </div>
      ...
      <div
        ...
        className="rounded-lg border border-viaduct-border overflow-hidden"
      >
        {/* Column header row */}
        <div
          role="rowgroup"
          className="bg-viaduct-background/50 border-b border-viaduct-border"
        >
          {/* Desktop header */}
          <div
            role="row"
        ...
        {/* Loading skeleton */}
        {isLoading && (
          <div role="rowgroup" className="divide-y divide-viaduct-border/50">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
