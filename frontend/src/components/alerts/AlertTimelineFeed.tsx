...
function AlertCard({ incident }: { readonly incident: BridgeIncident }) {
  ...
  return (
    <div className={`${style.border} ${style.bg} rounded-lg p-4 border border-viaduct-border/50 bg-viaduct-card/50`}>
      <div className="flex gap-3">
        <div className="flex-shrink-0 pt-1 text-lg">{style.icon}</div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h4 className="font-medium text-viaduct-text-primary truncate">{incident.title}</h4>
              <p className="text-sm text-viaduct-text-secondary mt-1">{incident.description}</p>
            </div>
          ...
          {(incident.sourceRepository || incident.sourceActor || incident.sourceType) && (
            <div className="flex flex-wrap gap-2 mt-2">
              {incident.sourceType && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-viaduct-background/40 text-viaduct-text-secondary text-xs rounded border border-viaduct-border/50">
                  {incident.sourceType}
                </span>
              )}
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="text-xs text-viaduct-text-secondary">{relativeTime}</span>
            {incident.assetCode && (
              <span className="text-xs text-viaduct-text-secondary">{incident.assetCode}</span>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-lg font-semibold text-viaduct-text-primary">Alert Timeline</h3>
          <p className="text-sm text-viaduct-text-secondary mt-1">Recent alerts and incidents from bridge operations</p>
        </div>
        <button
          type="button"
          onClick={() => {}}
          className="rounded-md border border-viaduct-border px-3 py-2 text-sm text-viaduct-text-secondary hover:text-viaduct-text-primary transition-colors"
        >
          Refresh
        </button>
      </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${
              severityFilter === "all"
                ...
                : "border-viaduct-border text-viaduct-text-secondary hover:text-viaduct-text-primary"
            }`}
          >
            All
          </button>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="h-24 rounded-lg border border-viaduct-border bg-viaduct-card/50 animate-pulse"
            />
          ))}
        </div>
      ) : error ? (
        ...
        <div className="rounded-lg border border-viaduct-border bg-viaduct-card/50 p-8 text-center">
          <p className="text-viaduct-text-secondary">No alerts found</p>
        </div>
      ) : (
        <div className="space-y-3">
