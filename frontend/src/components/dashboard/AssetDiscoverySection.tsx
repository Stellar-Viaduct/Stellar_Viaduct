...
export default function AssetDiscoverySection({
  ...
  if (isLoading) {
    return <p className="text-viaduct-text-secondary">Loading assets...</p>;
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm text-viaduct-text-secondary">Layout</span>
          <div className="inline-flex rounded-full border border-viaduct-border p-0.5">
            <button
              type="button"
          ...
          <span className="text-sm text-viaduct-text-secondary lg:ml-2">Sort</span>
          <select
            value={assetSort}
            onChange={(e) =>
              setPreference("assetSort", e.target.value as "symbol" | "health")
            }
            className="rounded-md border border-viaduct-border bg-viaduct-card px-2 py-1 text-xs text-white focus:outline-none focus:ring-2 focus:ring-viaduct-accent"
      ...
      {processed.length === 0 ? (
        ...
        <div
          ref={parentRef}
          className="max-h-[min(70vh,900px)] overflow-auto rounded-xl border border-viaduct-border/60 pr-1"
          role="region"
                    ...
                    {row.map((asset) => (
                      <div key={asset.symbol} className="space-y-2">
                        ...
                        <Link to={`/assets/${asset.symbol}`} className="group block">
                          <div className="rounded-lg transition-shadow group-hover:shadow-lg group-hover:shadow-viaduct-accent/10">
                            <HealthScoreCard
                              symbol={asset.symbol}
                              name={asset.name}
                          ...
                          {asset.health ? (
                            <p className="mt-2 px-1 text-xs text-viaduct-text-secondary md:hidden">
