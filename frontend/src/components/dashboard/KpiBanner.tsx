  ...
  return (
    <section aria-labelledby="dashboard-kpis" className="space-y-3">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <span className="rounded-full border border-viaduct-border px-3 py-1 text-xs text-viaduct-text-secondary">
      ...
      >
        {loading
          ...
          : items.map((item) => (
              <article
                key={item.id}
                className="group min-h-32 rounded-lg border border-viaduct-border bg-viaduct-card p-4 transition-colors hover:border-viaduct-accent"
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-xs font-medium uppercase text-viaduct-text-secondary">
                    {item.label}
                  </span>
                  <div className="flex items-center gap-2">
                    ...
                    {onInspectMetric ? (
                      <button
                        ...
                        className="inline-flex min-h-10 min-w-10 items-center justify-center rounded-md text-viaduct-text-secondary hover:bg-viaduct-surface hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
                ...
                {expanded ? (
                  <p className="mt-3 text-sm text-viaduct-text-secondary">{item.description}</p>
                ) : null}
                <button
                  type="button"
                  onClick={() => onDrilldown(item)}
                  className="mt-3 inline-flex min-h-10 items-center rounded-md text-xs font-medium text-viaduct-accent group-hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
