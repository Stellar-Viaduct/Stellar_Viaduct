...
const toneClasses: Record<StatusTone, string> = {
  ...
  unknown: "border-viaduct-border bg-viaduct-background/50 text-viaduct-text-secondary",
};
  ...
  return (
    <section aria-labelledby="inline-status-cards" className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <h2 id="inline-status-cards" className="text-sm font-semibold uppercase text-viaduct-text-secondary">
          Live status
        </h2>
        <span className="text-xs text-viaduct-text-secondary">Auto-refreshing</span>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {loading
          ...
          : items.map((item) => (
              <article
                key={item.id}
                className="min-h-28 rounded-lg border border-viaduct-border bg-viaduct-card p-3"
                aria-label={`${item.label}: ${item.value}. ${item.detail}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <p className="text-xs font-medium uppercase text-viaduct-text-secondary">
                    {item.label}
                  </p>
                  <span
                ...
                <p className="mt-2 text-lg font-semibold text-viaduct-text-primary">{item.value}</p>
                <p className="mt-1 text-sm text-viaduct-text-secondary">{item.detail}</p>
              </article>
            ))}
      </div>
    </section>
