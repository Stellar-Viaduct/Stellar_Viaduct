...
export default function DashboardTour({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100]" aria-hidden={false}>
      ...
      {rect ? (
        <div
          className="pointer-events-none absolute rounded-lg ring-2 ring-viaduct-accent transition-all duration-200"
          style={{
      ...
      <div
        ...
        tabIndex={-1}
        className="absolute w-80 rounded-xl border border-viaduct-border bg-viaduct-card p-4 shadow-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
        style={{ top: tooltipTop, left: tooltipLeft }}
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-medium uppercase tracking-wide text-viaduct-text-secondary">
            Step {activeStep + 1} of {steps.length}
          </span>
          <button
            type="button"
            onClick={onSkip}
            className="text-xs text-viaduct-text-secondary hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
          >
            Skip
          </button>
        </div>
        ...
        <p id={bodyId} className="mt-1 text-sm text-viaduct-text-secondary" aria-live="polite">
          {step.body}
        </p>
          ...
          {steps.map((dot, index) => (
            <span
              ...
              className={`h-1.5 rounded-full transition-all ${
                index === activeStep ? "w-4 bg-viaduct-accent" : "w-1.5 bg-stellar-border"
              }`}
            />
          ))}
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          <button
            ...
            onClick={onPrev}
            disabled={isFirst}
            className="rounded-md border border-viaduct-border px-3 py-1.5 text-sm text-viaduct-text-secondary transition-colors hover:text-white disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
          >
            Back
          </button>
          <button
            ...
            className="rounded-md border border-viaduct-accent bg-viaduct-accent/20 px-4 py-1.5 text-sm font-medium text-white transition-colors hover:bg-viaduct-accent/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-viaduct-accent"
          >
            {isLast ? "Finish" : "Next"}
          </button>
        </div>
