...
const STORAGE_KEY = "stellar-viaduct.export-picker.v1";
...
export default function ExportPickerDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 px-4 py-6">
      <div className="relative mx-auto w-full max-w-4xl rounded-3xl border border-viaduct-border bg-viaduct-card shadow-2xl">
        <div className="flex flex-col gap-6 p-6 lg:p-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded-full border border-viaduct-border bg-viaduct-background/90 p-2 text-viaduct-text-secondary transition-colors hover:bg-stellar-border hover:text-white"
          ...
          <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
            <div className="space-y-6">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-viaduct-border bg-viaduct-background/80 px-3 py-3 text-sm text-white outline-none focus:border-viaduct-accent focus:ring-2 focus:ring-viaduct-accent/30"
                  >
              ...
              <div className="space-y-3 rounded-3xl border border-viaduct-border bg-viaduct-background/70 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="rounded-full border border-viaduct-border px-3 py-1 text-xs text-viaduct-text-secondary hover:text-white"
                  >
                    Clear
                  </button>
                </div>
                <select
                  ...
                  className="h-full min-h-[10rem] w-full rounded-2xl border border-viaduct-border bg-viaduct-background/80 px-3 py-3 text-sm text-white outline-none focus:border-viaduct-accent focus:ring-2 focus:ring-viaduct-accent/30"
                >
            ...
            <div className="space-y-6 rounded-3xl border border-viaduct-border bg-viaduct-background/80 p-5 text-sm">
              <div className="space-y-3">
                <h3 className="text-base font-semibold text-white">Export preview</h3>
                <div className="grid gap-3">
                  <div className="rounded-2xl bg-viaduct-card/80 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-viaduct-text-secondary">Format</p>
                    <p className="mt-2 font-medium text-white">{formatOptions.find((item) => item.value === preferences.format)?.label}</p>
                  </div>
                  <div className="rounded-2xl bg-viaduct-card/80 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-viaduct-text-secondary">Scope</p>
                    <p className="mt-2 font-medium text-white">{dataTypeOptions.find((item) => item.value === preferences.dataType)?.label}</p>
                  </div>
                  <div className="rounded-2xl bg-viaduct-card/80 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-viaduct-text-secondary">Date range</p>
                    <p className="mt-2 text-white">{preferences.startDate} – {preferences.endDate}</p>
                  </div>
                  <div className="rounded-2xl bg-viaduct-card/80 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-viaduct-text-secondary">Asset filters</p>
                    <p className="mt-2 text-white">
                      {selectedAssetLabels.length > 0 ? selectedAssetLabels.map((asset) => asset.symbol).join(", ") : "All assets"}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-viaduct-card/80 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-viaduct-text-secondary">Bridge filters</p>
                    <p className="mt-2 text-white">
              ...
              <div className="space-y-3 rounded-3xl border border-viaduct-border bg-viaduct-card/80 p-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-white">Latest export</p>
                  {activeExport ? (
                    <span className="rounded-full bg-viaduct-background px-3 py-1 text-xs font-semibold text-viaduct-text-secondary">
                ...
                {activeExport ? (
                  <div className="space-y-3">
                    <p className="text-xs uppercase tracking-[0.18em] text-viaduct-text-secondary">Requested</p>
                    ...
                    {downloadUrl && activeExport.status === "completed" ? (
                      <a
                        href={downloadUrl}
                        className="inline-flex items-center justify-center rounded-xl bg-viaduct-accent px-4 py-3 text-sm font-semibold text-white transition hover:bg-viaduct-accent/90"
                      >
                        Download export
                      </a>
                    ) : (
              ...
              <div className="space-y-2 rounded-3xl border border-viaduct-border bg-viaduct-background/70 p-4">
                ...
                <div className="space-y-3">
                  {recentExports.length === 0 ? (
                    <p className="text-sm text-viaduct-text-secondary">No exports requested yet.</p>
                  ) : (
                    recentExports.map((record) => (
                      <div key={record.id} className="rounded-2xl border border-viaduct-border bg-viaduct-card/70 p-3">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-medium text-white">{record.data_type.replace("_", " ")}</p>
                          <span className="rounded-full bg-viaduct-background px-2 py-1 text-xs text-viaduct-text-secondary">
                            {formatExportStatus(record.status)}
                          </span>
                        </div>
                        <p className="text-xs text-viaduct-text-secondary">{new Date(record.created_at).toLocaleString()}</p>
                      </div>
                    ))
                  )}
                </div>
          ...
          <div className="flex flex-col gap-3 border-t border-viaduct-border pt-4 sm:flex-row sm:justify-end">
            <button
              type="button"
              onClick={onClose}
              className="rounded-2xl border border-viaduct-border bg-viaduct-background/90 px-4 py-3 text-sm text-white transition hover:bg-stellar-border"
            >
              Close
            </button>
            <button
              ...
              className="inline-flex justify-center rounded-2xl bg-viaduct-accent px-4 py-3 text-sm font-semibold text-white transition hover:bg-viaduct-accent/90 disabled:cursor-not-allowed disabled:opacity-50"
            >
