...
const STORAGE_KEY = "stellarviaduct:recent_actions";
...
export default function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-24 px-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-xl bg-viaduct-card border border-viaduct-border rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="max-h-80 overflow-y-auto">
          {recent.length > 0 && query.trim() === "" && (
            <div className="px-3 py-2 text-xs text-viaduct-text-secondary">Recent</div>
          )}
          <ul>
            {visibleItems.map((a) => (
              <li key={a.id} className="px-3 py-2 hover:bg-stellar-border/60 cursor-pointer" onClick={() => execute(a)}>
                <div className="text-sm text-viaduct-text-primary">{a.title}</div>
                <div className="text-xs text-viaduct-text-secondary">{a.href}</div>
              </li>
            ))}
          </ul>
        </div>
