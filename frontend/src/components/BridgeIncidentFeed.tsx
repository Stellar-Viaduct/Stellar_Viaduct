...
function IncidentCard({ incident, isUnread, onMarkRead }: IncidentCardProps) {
  ...
  return (
    <article
      className={`border rounded-lg p-4 transition-colors cursor-pointer hover:bg-viaduct-card-hover ${
        isUnread
          ? "border-viaduct-border bg-viaduct-card"
          : "border-viaduct-border/50 bg-viaduct-card/50 opacity-80"
      }`}
      onClick={() => {
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-grow min-w-0">
          ...
          <h3 className={`text-sm font-semibold truncate ${isUnread ? "text-white" : "text-viaduct-text-secondary"}`}>
            {incident.title}
          </h3>

          <div className="flex flex-wrap gap-2 mt-1 items-center">
            <span className="text-xs text-stellar-text-muted">
              Bridge: <span className="text-viaduct-text-secondary">{incident.bridgeId}</span>
            </span>
            {incident.assetCode && (
              <span className="text-xs text-stellar-text-muted">
                Asset: <span className="text-viaduct-text-secondary">{incident.assetCode}</span>
              </span>
            )}
            {incident.sourceRepository && (
              <span className="text-xs text-stellar-text-muted inline-flex items-center gap-1">
                ...
                Repo: <span className="text-viaduct-text-secondary">{incident.sourceRepository}</span>
              </span>
            )}
          </div>
          ...
          {expanded && (
            <div className="mt-3 space-y-3">
              <p className="text-sm text-viaduct-text-secondary">{incident.description}</p>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {incident.followUpActions.map((action, i) => (
                      <li key={i} className="text-sm text-viaduct-text-secondary flex gap-2">
                        <span className="text-viaduct-accent">•</span>
                        {action}
                      </li>
                    ))}
                  </ul>
              ...
              {(incident.sourceUrl || incident.sourceType || incident.sourceActor) && (
                <div className="flex flex-wrap items-center gap-2 text-xs text-stellar-text-muted">
                  {incident.sourceType && (
                    <span className="uppercase tracking-wide px-2 py-0.5 rounded border border-viaduct-border bg-viaduct-card">
                      {incident.sourceType}
                    </span>
                  )}
                  ...
                  {incident.sourceUrl && (
                    <a
                      ...
                      className="text-viaduct-accent hover:underline inline-flex items-center gap-1"
...
export default function BridgeIncidentFeed({
  ...
  const inputClass =
    "bg-viaduct-card border border-viaduct-border rounded px-3 py-1.5 text-sm text-white placeholder-stellar-text-muted focus:outline-none focus:border-viaduct-accent w-full sm:w-auto";
    ...
    <section className="space-y-4" aria-label="Bridge Incident Feed">
      ...
      {isLoading && (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="border border-viaduct-border rounded-lg p-4 animate-pulse bg-viaduct-card">
      ...
      {!isLoading && !error && sorted.length === 0 && (
        <div className="border border-viaduct-border rounded-lg p-8 text-center text-viaduct-text-secondary">
