...
function ServiceHealthPulseSkeleton({ className }: { className?: string }) {
  return (
    <div
      className={`bg-viaduct-card border border-viaduct-border rounded-lg p-4 ${className || ""}`}
      role="article"
...
export default function ServiceHealthPulse({
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-viaduct-text-primary">
  ...
  return (
    <div
      className={`bg-viaduct-card border border-viaduct-border rounded-lg p-4 hover:border-viaduct-accent transition-colors ${className}`}
      role="article"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-medium text-viaduct-text-primary truncate">
              {config.label}
            </p>
            {data.totalServices > 0 && (
              <button
                ...
                className="flex items-center justify-center w-5 h-5 rounded hover:bg-stellar-border focus:outline-none focus:ring-2 focus:ring-viaduct-accent transition-colors"
                aria-label={isExpanded ? "Collapse service details" : "Expand service details"}
              >
                <svg
                  className={`w-3 h-3 text-viaduct-text-secondary transition-transform duration-200 ${
          ...
          <div className="flex items-center gap-2 mt-1 text-xs text-viaduct-text-secondary">
      ...
      {data.totalServices > 0 && (
        ...
        >
          <div className="overflow-hidden">
            <div className="pt-3 mt-3 border-t border-viaduct-border">
                  ...
                  return (
                    ...
                    >
                      <span
                        ...
                        aria-hidden="true"
                      />
                      <span className="flex-1 text-sm text-viaduct-text-primary truncate">
                        {service.name}
                      </span>
                      <span
