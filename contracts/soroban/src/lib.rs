...
// tests (native target) to avoid Wasm symbol conflicts with StellarViaductContract.
pub mod acl;
#[cfg(test)]
...
#[contract]
pub struct StellarViaductContract;

#[allow(clippy::too_many_arguments)]
#[contractimpl]
impl StellarViaductContract {
    ...
    pub fn submit_health_batch(env: Env, caller: Address, records: Vec<HealthScoreBatch>) {
        ...
        for item in records.iter() {
            ...
            Self::emit_contract_event(
                &env,
                StellarViaductEvent::HealthSubmitted {
                    actor: caller.clone(),
                    asset_code: item.asset_code.clone(),
                    health_score: item.health_score,
                    timestamp,
    ...
    pub fn set_deviation_threshold(
        ...
        Self::emit_contract_event(
            &env,
            StellarViaductEvent::ThresholdUpdated {
                actor: admin,
    ...
    pub fn grant_role(env: Env, granter: Address, grantee: Address, role: AdminRole) {
        ...
        Self::emit_contract_event(
            &env,
            StellarViaductEvent::RoleChanged {
                actor: granter,
                target: grantee,
                granted: true,
                role,
    ...
    pub fn set_expiration_policy(
        ...
        Self::emit_contract_event(
            &env,
            StellarViaductEvent::ExpirationPolicyUpdated {
                actor: caller,
    ...
    pub fn extend_expiration(env: Env, caller: Address, asset_code: String, extra_secs: u64) {
            ...
            .get::<_, HealthScoreResult>(&AssetDataKey::HealthRes(asset_code.clone()))
        {
            ...
            Self::emit_contract_event(
                &env,
                StellarViaductEvent::ExpirationExtended {
                    actor: caller,
                    scope: asset_code,
    ...
    pub fn cleanup_expired_data(env: Env, caller: Address, max_records: u32) -> CleanupStats {
        ...
        Self::emit_contract_event(
            &env,
            StellarViaductEvent::CleanupCompleted {
                actor: caller,
                removed_records,
                trimmed_history_records,
                timestamp: now,
    ...
    fn emit_contract_event(env: &Env, event: StellarViaductEvent) {
        match event {
            StellarViaductEvent::HealthSubmitted {
                actor,
                asset_code,
...
mod tests {
    ...
    fn setup() -> (Env, StellarViaductContractClient<'static>, Address) {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, StellarViaductContract);
        let client = StellarViaductContractClient::new(&env, &contract_id);
