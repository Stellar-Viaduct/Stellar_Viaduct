...
This document describes the implementation of the Trusted Source Registry feature for the Stellar Viaduct Soroban contract. This feature provides an additional layer of access control for contract submissions by maintaining a registry of trusted external sources.
...
```bash
# Run all tests
cargo test --package stellar-viaduct-soroban

# Run only source trust tests
cargo test --package stellar-viaduct-soroban --test source_trust

# Run with output
cargo test --package stellar-viaduct-soroban --test source_trust -- --nocapture
```
