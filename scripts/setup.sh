#!/usr/bin/env bash
# =============================================================================
# Stellar Viaduct — Development Environment Setup Script
#
...
show_help() {
  cat <<'HELP'
Stellar Viaduct — Development Environment Setup
...
load_runtime_env_defaults() {
  ...
  set_env_default "POSTGRES_USER" "stellar_viaduct"
  set_env_default "POSTGRES_DB" "stellar_viaduct"
...
wait_for_service() {
  ...
  while [[ $elapsed -lt $timeout ]]; do
    ...
    if [[ "$service" == "postgres" ]]; then
      if docker exec stellar-viaduct-postgres pg_isready -U "${POSTGRES_USER:-stellar_viaduct}" &>/dev/null; then
        # pg_isready succeeds before init scripts finish — verify with an actual query
        if docker exec stellar-viaduct-postgres psql -U "${POSTGRES_USER:-stellar_viaduct}" -d "${POSTGRES_DB:-stellar_viaduct}" -c "SELECT 1" &>/dev/null; then
          success "${service} is healthy (verified via query)"
          return 0
        fi
      fi
    elif [[ "$service" == "redis" ]]; then
      if docker exec stellar-viaduct-redis redis-cli ping 2>/dev/null | grep -q PONG; then
  ...
  die "${service} did not become healthy within ${timeout}s. Check: docker logs stellar-viaduct-${service}"
}
...
print_summary() {
  ...
  printf "${BOLD}${GREEN}  ✔  Stellar Viaduct — Setup Complete${RESET}\n"
  printf "${BOLD}${GREEN}%s${RESET}\n" "════════════════════════════════════════════════════════"
  echo ""
  info "Next steps:"
  echo ""
  ...
  dim "    PgAdmin    http://localhost:${PGADMIN_PORT:-5050}  (admin@stellarviaduct.io / admin)"
...
main() {
  ...
  printf "${BOLD}${CYAN}║     Stellar Viaduct — Development Environment Setup    ║${RESET}\n"
  printf "${BOLD}${CYAN}╚══════════════════════════════════════════════════════╝${RESET}\n"
  echo ""
