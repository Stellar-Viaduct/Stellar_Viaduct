...
The webhook delivery system enables external services to receive real-time notifications about StellarViaduct events. When an alert triggers, a bridge status changes, or significant events occur, registered webhook endpoints receive a signed HTTP POST request with the event data.
...
| `User-Agent` | `StellarViaduct-Webhook/1.0` |
...
```json
{
  ...
  "timestamp": "2026-03-28T12:00:00.000Z",
  "data": {
    "message": "This is a test webhook delivery from StellarViaduct",
    "webhookEndpointId": "uuid",
    "test": true
  }
}
