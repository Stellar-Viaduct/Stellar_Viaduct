...
The Stellar Viaduct backend requires a Node.js HTTP framework that handles high-frequency API requests for monitoring data, supports JSON schema validation natively, and provides good performance for serializing large JSON responses containing time-series data.
