...
Stellar Viaduct ingests high-frequency time-series data: price observations every 30 seconds, health scores every minute, and liquidity snapshots every minute. This data needs efficient storage, fast time-range queries, and automatic lifecycle management (retention, compression).
