...
This document describes the comprehensive rate limiting and throttling system implemented for Stellar Viaduct API endpoints.
...
class RateLimitedAPIClient {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseURL = 'https://api.stellar-viaduct.com';
  }
...
# prometheus.yml
scrape_configs:
  - job_name: 'stellar-viaduct-rate-limits'
    static_configs:
  ...
  - name: rate-limiting
    rules:
      - alert: HighRateLimitBlockRate
        expr: stellar_viaduct_rate_limit_block_rate > 0.1
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: RateLimitBurstTraffic
        expr: stellar_viaduct_rate_limit_burst_ratio > 0.8
        for: 2m
        labels:
          severity: critical
        annotations:
