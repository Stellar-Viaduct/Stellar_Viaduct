...
This document describes the comprehensive health check endpoints implemented for Stellar Viaduct, designed for monitoring, alerting, and Kubernetes orchestration.
...
```
# HELP stellar_viaduct_health_status Health check status (1=healthy, 0.5=degraded, 0=unhealthy)
# TYPE stellar_viaduct_health_status gauge
stellar_viaduct_health_status{component="database"} 1
stellar_viaduct_health_status{component="redis"} 1
stellar_viaduct_health_status{component="external_apis"} 1
stellar_viaduct_health_status{component="system"} 1
stellar_viaduct_health_status{component="overall"} 1

# HELP stellar_viaduct_uptime_seconds Application uptime in seconds
# TYPE stellar_viaduct_uptime_seconds counter
stellar_viaduct_uptime_seconds 3600.123

# HELP stellar_viaduct_health_check_duration_seconds Health check duration in seconds
# TYPE stellar_viaduct_health_check_duration_seconds gauge
stellar_viaduct_health_check_duration_seconds{component="database"} 0.045
stellar_viaduct_health_check_duration_seconds{component="redis"} 0.012
stellar_viaduct_health_check_duration_seconds{component="external_apis"} 1.234
stellar_viaduct_health_check_duration_seconds{component="system"} 0.005
```
...
- `stellar_viaduct_health_status` - Overall system health
- `stellar_viaduct_uptime_seconds` - Application uptime
- `stellar_viaduct_health_check_duration_seconds` - Health check performance
...
groups:
  - name: stellar-viaduct
    rules:
      - alert: StellarViaductUnhealthy
        expr: stellar_viaduct_health_status{component="overall"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "Stellar Viaduct is unhealthy"
          
      - alert: StellarViaductDegraded
        expr: stellar_viaduct_health_status{component="overall"} == 0.5
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "Stellar Viaduct is degraded"
          
      - alert: DatabaseDown
        expr: stellar_viaduct_health_status{component="database"} == 0
        for: 30s
        labels:
          severity: critical
        annotations:
      ...
      - alert: RedisDown
        expr: stellar_viaduct_health_status{component="redis"} == 0
        for: 30s
        labels:
          severity: critical
        annotations:
...
3. **High Memory Usage**
   - Monitor `stellar_viaduct_health_check_duration_seconds`
