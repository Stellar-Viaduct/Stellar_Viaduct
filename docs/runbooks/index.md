# Stellar Viaduct Runbook Index

This index is the single entry point for all operational runbooks covering the Stellar Viaduct platform — a cross-chain asset health monitoring system built on Stellar and Soroban. Runbooks document repeatable procedures so that any on-call engineer can execute critical tasks safely, consistently, and without relying on tribal knowledge.
...
5. **Reach out to external parties** if the situation involves a third-party dependency (Circle API, Stellar network, Horizon, Soroban RPC):
   ...
   - Stellar Viaduct security contact: security@stellarviaduct.io
