...
This playbook defines the end-to-end procedure for rotating every secret used by the Stellar Viaduct platform — a cross-chain asset health monitoring system built on Stellar and Soroban. It covers pre-rotation preparation, per-secret rotation steps, service impact expectations, rollback procedures, audit requirements, and the emergency path for a confirmed compromise.
...
2. Connect to the PostgreSQL instance as a superuser:
   ...
   ```
3. Update the `stellar_viaduct` database user's password:
   ```sql
   ALTER USER stellar_viaduct WITH PASSWORD '<new-password>';
   ```
4. Update the secret in the appropriate storage location:
   - **Docker / local:** Replace `POSTGRES_PASSWORD` in `.env`.
   - **Kubernetes:** `kubectl create secret generic stellar-viaduct-secrets --namespace stellar-viaduct --from-literal=postgres-password=<new-password> --dry-run=client -o yaml | kubectl apply -f -`
...
5. Perform a rolling restart of the backend service to pick up the new credential:
   ...
   - **Kubernetes:** `kubectl rollout restart deployment/stellar-viaduct-backend -n stellar-viaduct`
...
5. Restart the reserve verification worker:
   ...
   - **Kubernetes:** `kubectl rollout restart deployment/stellar-viaduct-worker -n stellar-viaduct`
...
2. **Pause the worker** for the affected bridge:
   ...
   - **Kubernetes:** `kubectl scale deployment/stellar-viaduct-worker --replicas=0 -n stellar-viaduct`
...
7. **Resume the worker:**
   ...
   - **Kubernetes:** `kubectl scale deployment/stellar-viaduct-worker --replicas=1 -n stellar-viaduct`
...
2. **PagerDuty:** In PagerDuty, navigate to **Services** > **Integrations** and regenerate the integration key for the Stellar Viaduct service.
