...
This guide explains how API authentication works in Stellar Viaduct, including API key methods, scope design, token lifecycle, and common failure modes.

## Authentication Methods

Stellar Viaduct currently supports API-key authentication for protected REST endpoints.
...
curl -X GET http://localhost:3000/api/v1/jobs/status \
  -H "x-api-key: $STELLAR_VIADUCT_API_KEY"
```
...
Stellar Viaduct enforces allow-list scopes at route level. A key must include every scope required by the target route.
