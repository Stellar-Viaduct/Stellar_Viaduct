...
This guide covers load balancer and reverse proxy configuration for Stellar Viaduct.

## Overview

Stellar Viaduct uses Nginx as the default reverse proxy in production. The frontend container includes a pre-configured Nginx instance that handles:
...
Any load balancer in front of Stellar Viaduct must support:
...
Listener Rules:
  - HTTPS:443
    - Host: api.StellarViaduct.dev, Path: /api/v1/ws → stellar-viaduct-ws
    - Host: api.StellarViaduct.dev, Path: /api/* → stellar-viaduct-api
    - Host: StellarViaduct.dev → stellar-viaduct-frontend
```
...
server {
    listen 443 ssl;
    server_name api.StellarViaduct.dev;
...
server {
    listen 443 ssl;
    server_name StellarViaduct.dev;
...
Stellar Viaduct provides dedicated health check endpoints optimized for load balancer probes:
