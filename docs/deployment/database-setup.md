...
This guide covers PostgreSQL with TimescaleDB setup, migrations, seeding, and maintenance for Stellar Viaduct.

## Overview

Stellar Viaduct uses **PostgreSQL 15+** with the **TimescaleDB** extension for efficient time-series data storage. The database stores both relational data (assets, bridges, configurations) and high-frequency time-series data (prices, health scores, liquidity snapshots).
...
sudo -u postgres psql -c "CREATE USER STELLAR_VIADUCT WITH PASSWORD 'your_password';"
sudo -u postgres psql -c "CREATE DATABASE STELLAR_VIADUCT OWNER STELLAR_VIADUCT;"
sudo -u postgres psql -d STELLAR_VIADUCT -c "CREATE EXTENSION IF NOT EXISTS timescaledb;"
```
...
# Via Docker
docker compose exec postgres psql -U STELLAR_VIADUCT -d STELLAR_VIADUCT

# Kubernetes
kubectl exec -it statefulset/stellar-viaduct-postgres -n stellar-viaduct -- \
  psql -U STELLAR_VIADUCT -d STELLAR_VIADUCT
```
...
SELECT count(*) FROM pg_stat_activity WHERE datname = 'STELLAR_VIADUCT';
```
