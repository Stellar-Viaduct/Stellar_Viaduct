...
Common issues, debugging steps, and operational runbooks for Stellar Viaduct.
...
docker compose exec postgres pg_isready -U STELLAR_VIADUCT
...
3. **Connection from wrong network:** Ensure the backend is on the `stellar-viaduct` network
...
# Kubernetes
kubectl scale deployment stellar-viaduct-backend --replicas=5 -n stellar-viaduct
```
...
docker compose exec -T postgres pg_restore \
  -U STELLAR_VIADUCT -d STELLAR_VIADUCT --clean --if-exists \
...
docker compose exec postgres psql -U STELLAR_VIADUCT -c \
  "ALTER USER STELLAR_VIADUCT WITH PASSWORD '${NEW_PASSWORD}';"
...
1. Search [existing issues](https://github.com/Stellar_Viaduct/Stellar_Viaduct/issues) for similar problems
2. Check the [discussions](https://github.com/Stellar_Viaduct/Stellar_Viaduct/discussions) for community solutions
