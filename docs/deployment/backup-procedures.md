...
This guide covers backup and disaster recovery procedures for Stellar Viaduct.

## Backup Strategy

Stellar Viaduct requires backups for two primary data stores:
...
docker compose exec postgres pg_dump \
  ...
  --format=custom \
  --compress=9 \
  > backups/STELLAR_VIADUCT_$(date +%Y%m%d_%H%M%S).dump

# Kubernetes environment
kubectl exec -it statefulset/stellar-viaduct-postgres -n stellar-viaduct -- \
  pg_dump -U STELLAR_VIADUCT -d STELLAR_VIADUCT --format=custom --compress=9 \
  > backups/STELLAR_VIADUCT_$(date +%Y%m%d_%H%M%S).dump
```
...
BACKUP_FILE="${BACKUP_DIR}/STELLAR_VIADUCT_${TIMESTAMP}.dump"
...
find "$BACKUP_DIR" -name "STELLAR_VIADUCT_*.dump" -mtime +"$RETENTION_DAYS" -delete
...
0 2 * * * cd /opt/stellar-viaduct && bash scripts/backup-db.sh >> /var/log/stellar-viaduct-backup.log 2>&1
```
...
docker compose exec -T postgres pg_restore \
  ...
  --clean \
  --if-exists \
  < backups/STELLAR_VIADUCT_20260329_020000.dump
...
```bash
# Restore base backup
pg_restore --clean --if-exists -d STELLAR_VIADUCT base_backup.dump
...
```bash
# Upload backup to S3
aws s3 cp backups/STELLAR_VIADUCT_latest.dump \
  s3://stellar-viaduct-backups/$(date +%Y/%m/%d)/ \
...
# Copy RDB file
docker cp stellar-viaduct-redis:/data/dump.rdb backups/redis_$(date +%Y%m%d).rdb
```
...
docker compose exec postgres createdb -U STELLAR_VIADUCT STELLAR_VIADUCT_test

# Restore backup to test database
docker compose exec -T postgres pg_restore \
  -U STELLAR_VIADUCT \
  -d STELLAR_VIADUCT_test \
  < backups/STELLAR_VIADUCT_latest.dump

# Verify data integrity
docker compose exec postgres psql -U STELLAR_VIADUCT -d STELLAR_VIADUCT_test \
  -c "SELECT count(*) FROM assets; SELECT count(*) FROM bridges;"

# Clean up
docker compose exec postgres dropdb -U STELLAR_VIADUCT STELLAR_VIADUCT_test
```
