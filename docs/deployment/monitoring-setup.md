...
This guide covers setting up monitoring, alerting, and observability for Stellar Viaduct.

## Overview

Stellar Viaduct provides built-in monitoring capabilities:
...
Stellar Viaduct exposes five health endpoints on the backend (port 3001):
...
scrape_configs:
  - job_name: 'stellar-viaduct-backend'
    metrics_path: '/health/metrics'
    static_configs:
      - targets: ['backend:3001']
    scrape_interval: 15s

  - job_name: 'stellar-viaduct-postgres'
    static_configs:
      - targets: ['postgres-exporter:9187']

  - job_name: 'stellar-viaduct-redis'
    static_configs:
...
services:
  prometheus:
    ...
    container_name: stellar-viaduct-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: stellar-viaduct-grafana
    ports:
      - "3000:3000"
    environment:
...
Stellar Viaduct uses **Pino** for structured JSON logging to stdout:
...
```json
{
  ...
  "time": 1711713600000,
  "pid": 1,
  "hostname": "stellar-viaduct-backend",
...
output.elasticsearch:
  ...
  index: "stellar-viaduct-%{+yyyy.MM.dd}"
```
...
Stellar Viaduct includes request tracing middleware that assigns a unique correlation ID to every request:
...
```yaml
groups:
  - name: stellar-viaduct-alerts
    rules:
      - alert: StellarViaductDown
        expr: up{job="stellar-viaduct-backend"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "Stellar Viaduct backend is down"

      - alert: HighMemoryUsage
        expr: process_resident_memory_bytes{job="stellar-viaduct-backend"} > 450e6
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "Stellar Viaduct memory usage is high"

      - alert: HighErrorRate
        expr: rate(http_requests_total{job="stellar-viaduct-backend",status=~"5.."}[5m]) > 0.01
        for: 5m
        labels:
          severity: warning
        annotations:
