...
Stellar Viaduct uses environment variables for all runtime configuration. A `.env.example` file is provided in the repository root.
...
| `POSTGRES_DB` | `STELLAR_VIADUCT` | Yes | Database name |
| `POSTGRES_USER` | `STELLAR_VIADUCT` | Yes | Database user |
| `POSTGRES_PASSWORD` | `STELLAR_VIADUCT_dev` | Yes | Database password |
...
| Variable | Default | Required | Description |
|----------|---------|----------|-------------|
| `PGADMIN_EMAIL` | `admin@stellarviaduct.io` | No | PgAdmin login email |
...
POSTGRES_PASSWORD=STELLAR_VIADUCT_dev
...
VITE_API_URL=https://staging-api.StellarViaduct.dev
VITE_WS_URL=wss://staging-api.StellarViaduct.dev/ws
```
...
VITE_API_URL=https://api.StellarViaduct.dev
VITE_WS_URL=wss://api.StellarViaduct.dev/ws
```
