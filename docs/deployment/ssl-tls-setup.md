...
This guide covers configuring HTTPS and secure communication for Stellar Viaduct deployments.
...
services:
  nginx-proxy:
    ...
    container_name: stellar-viaduct-proxy
    ports:
      - "80:80"
      - "443:443"
    volumes:
  ...
  certbot:
    ...
    container_name: stellar-viaduct-certbot
    volumes:
...
server {
    listen 80;
    server_name StellarViaduct.dev api.StellarViaduct.dev;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/StellarViaduct.dev/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/StellarViaduct.dev/privkey.pem;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/api.StellarViaduct.dev/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.StellarViaduct.dev/privkey.pem;
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers HIGH:!aNULL:!MD5;
    ssl_prefer_server_ciphers on;
...
docker compose -f docker-compose.yml -f docker-compose.ssl.yml run --rm certbot \
  ...
  -d StellarViaduct.dev \
  -d api.StellarViaduct.dev \
