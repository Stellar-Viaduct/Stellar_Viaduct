...
Stellar Viaduct uses **Knex.js** as its migration and query-builder layer on top of
...
For test environments set `POSTGRES_DB=stellar_viaduct_test` to isolate test data.
...
```bash
psql -U stellar_viaduct stellar_viaduct < scripts/init-db.sql
```
