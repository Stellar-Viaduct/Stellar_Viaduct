...
This document describes the comprehensive request logging and distributed tracing system implemented for Stellar Viaduct API.
...
LOG_FILE=/var/log/stellar-viaduct.log     # Log file path (optional)
...
filter {
  if [fields][service] == "stellar-viaduct-api" {
    json {
      source => "message"
    }
...
output {
  elasticsearch {
    ...
    index => "stellar-viaduct-logs-%{+YYYY.MM.dd}"
  }
}
```

#### Kibana Index Pattern
- Index pattern: `stellar-viaduct-logs-*`
